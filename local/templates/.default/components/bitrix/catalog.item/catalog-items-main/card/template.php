...
<h3 class="product-item-title">
	<? if ($itemHasDetailUrl): ?>
		<a href="<?= $item['DETAIL_PAGE_URL'] ?>" title="<?= $productTitle ?>" class="truncate-2">
		<? endif; ?>
		<?= $productTitle ?>
		<? if ($itemHasDetailUrl): ?>
		</a>
...
if (!empty($arParams['PRODUCT_BLOCKS_ORDER'])) {
	foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName) {
		switch ($blockName) {
			case 'price': ?>
				<div class="product-item-info-container product-item-price-container my-3" data-entity="price-block">
					<?
					...
					?>
					<span class="product-item-price-current m-2" id="<?= $itemIds['PRICE'] ?>">
						<?
						if (!empty($price)) {
			...
			case 'buttons':
				?>
				<div class="product-item-info-container product-item-info-buttons product-item-hidden mx-2 my-3"
					data-entity="buttons-block">
					<?
					// --- Логика для простых товаров (без торговых предложений) ---
					if (!$haveOffers) {
						if ($actualItem['CAN_BUY']) {
							?>
							<div class="product-item-button-container" id="<?= $itemIds['BASKET_ACTIONS'] ?>">

								<div data-entity="add-to-cart-button" style="transition: opacity 0.2s ease, visibility 0.2s ease;">
									<button class="btn btn-primary main-btn-catalog <?= $buttonSizeClass ?>" id="<?= $itemIds['BUY_LINK'] ?>"
										href="javascript:void(0)" rel="nofollow">
										<i class="fas fa-shopping-cart mr-2"></i>
										<span
											class="text-btn-cart"><?= ($arParams['ADD_TO_BASKET_ACTION'] === 'BUY' ? $arParams['MESS_BTN_BUY'] : $arParams['MESS_BTN_ADD_TO_BASKET']) ?></span>
									</button>
								</div>

								<div class="product-item-detail-quantity-container" data-entity="quantity-block-in-cart" style="display: none;">
									<div class="product-quantity-container-in">
										<div class="product-quantity">
											<button class="product-quantity__button product-quantity__button--minus no-select" type="button"
												id="<?= $itemIds['QUANTITY_DOWN'] ?>">&minus;</button>
											<input class="product-quantity__input" type="number" id="<?= $itemIds['QUANTITY'] ?>"
												value="<?= $measureRatio ?>">
											<button class="product-quantity__button product-quantity__button--plus no-select" type="button"
												id="<?= $itemIds['QUANTITY_UP'] ?>">&plus;</button>
										</div>
									</div>
								</div>
								<div class="product-quantity-total-price-container">
									<div class="product-quantity-total-price" id="<?= $itemIds['PRICE_TOTAL'] ?>" style="display: none;">
									</div>
								</div>
							</div>
							<?
							...
							<div class="product-item-button-container">
								...
								?>
								<button class="btn btn-primary main-btn-catalog not-available  <?= $buttonSizeClass ?>"
					...
					// --- Логика для товаров с торговыми предложениями (SKU) ---
					else {
						if ($arParams['PRODUCT_DISPLAY_MODE'] === 'Y') {
							?>
							<div class="product-item-button-container">
								...
								<div id="<?= $itemIds['BASKET_ACTIONS'] ?>" <?= ($actualItem['CAN_BUY'] ? '' : 'style="display: none;"') ?>>
									...
									<div data-entity="add-to-cart-button" style="transition: opacity 0.2s ease, visibility 0.2s ease;">
										<button class="btn btn-primary main-btn-catalog <?= $buttonSizeClass ?>"
											id="<?= $itemIds['BUY_LINK'] ?>" href="javascript:void(0)" rel="nofollow">
											<i class="fas fa-shopping-cart mr-2"></i>
											<span
												class="text-btn-cart"><?= ($arParams['ADD_TO_BASKET_ACTION'] === 'BUY' ? $arParams['MESS_BTN_BUY'] : $arParams['MESS_BTN_ADD_TO_BASKET']) ?></span>
										</button>
											...
											<ul class="row d-flex product-item-scu-item-list">
												...
												foreach ($skuProperty['VALUES'] as $value) {
													...
													if ($skuProperty['SHOW_MODE'] === 'PICT') {
														...
														?>
														<li class="col-xl-3 col-sm-6 product-item-scu-item-text-container" title="<?= $value['NAME'] ?>"
															data-treevalue="<?= $propertyId ?>_<?= $value['ID'] ?>" data-onevalue="<?= $value['ID'] ?>">


															<div class="product-item-scu-item-text-block">
																<div class="d-flex justify-content-center product-item-scu-item-text">
																	<?= $value['NAME'] ?>
																</div>
															</div>
														</li>
