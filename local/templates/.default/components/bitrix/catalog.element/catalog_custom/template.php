...
$itemIds = array(
	...
	'SMALL_CARD_QUANTITY_ID' => $mainId . '_small_card_quantity',
	'SMALL_CARD_QUANTITY_UP_ID' => $mainId . '_small_card_quant_up',
	'SMALL_CARD_QUANTITY_DOWN_ID' => $mainId . '_small_card_quant_down',
	'SMALL_CARD_ADD_BTN_ID' => $mainId . '_small_card_add_btn',
);
	...
	<div class="<?= ($showBlockWithOffersAndProps ? "col-md-5 col-lg-6" : "col-md-4"); ?>">
		...
		<div class="row">
			...
			if ($showBlockWithOffersAndProps) {
				...
				<div class="col-lg-5">
					...
					if (!empty($arResult['OFFERS']) && is_array($arResult['OFFERS'])) {
						foreach ($arResult['OFFERS'] as $offer) {
							// Проверяем, что у оффера есть свойство WEIGHT_REFERENCE
							$hasWeightProperty = isset($offer['PROPERTIES']['WEIGHT_REFERENCE']['VALUE']) && !empty($offer['PROPERTIES']['WEIGHT_REFERENCE']['VALUE']);

							// Проверяем, что у оффера есть цена (ЗАЩИТА ОТ ОШИБКИ TypeError)
							$hasPrice = isset($offer['ITEM_PRICES']) && is_array($offer['ITEM_PRICES']) && !empty($offer['ITEM_PRICES']);

							if ($hasWeightProperty && $hasPrice) {
								$weightValueId = $offer['PROPERTIES']['WEIGHT_REFERENCE']['VALUE'];
								$priceValue = $offer['ITEM_PRICES'][0]['PRINT_PRICE'];
								$pricesByWeightId[$weightValueId] = $priceValue;
							}
						}
					}
					...
					foreach ($arParams['PRODUCT_INFO_BLOCK_ORDER'] as $blockName) {
						switch ($blockName) {
							case 'sku':
								if ($showOffersBlock) {
									?>
									<div class="mb-3 is-loading" id="<?= $itemIds['TREE_ID'] ?>">
										<?php
															...
															<ul class="product-item-scu-item-list">
																...
																foreach ($skuProperty['VALUES'] as &$value) {
																	...
																	if ($skuProperty['SHOW_MODE'] === 'PICT') {
																		...
																		// Определяем цену. По умолчанию она пустая.
																		$priceValue = '';

																		// Если это наше свойство с весом, пытаемся найти для него цену
																		if ($skuProperty['CODE'] === 'WEIGHT_REFERENCE') {
																			$currentWeightId = $value['ID'];
																			// Безопасно получаем цену из подготовленного ранее массива
																			$priceValue = $pricesByWeightId[$currentWeightId] ?? '';
																		}
																		?>
																		<li class="product-item-scu-item-text-container" title="<?= $value['NAME'] ?>"
																			...
																			<div class="product-item-scu-item-text-block">
																				<div class="product-item-scu-item-text">
																					<span
																						class="product-item-scu-item-value"><?= $value['NAME'] ?></span>
																					<?php if (!empty($priceValue)):  // Если цена нашлась, выводим её ?>
																						<span class="product-item-scu-item-price"><?= $priceValue ?></span>
																					<?php endif; ?>
																				</div>
																			</div>
																		</li>
			...
			<div class="<?= ($showBlockWithOffersAndProps ? "col-lg-7" : "col-lg"); ?>">
				<div class="product-item-art-block">

					<div class="product-item-art-block">
						<span class="product-item-detail-properties-name">
							<?= "Арт. " . $actualItem["PROPERTIES"]["ARTNUMBER"]["VALUE"]; ?>
						</span>
					</div>

					<div class="product-item-detail-pay-block is-loading">

						<div class="product-stock-status mb-2">
							<div class="stock-status stock-status--in" id="<?= $mainId ?>_stock_status_in"
								style="display: none;">
								В наличии
							</div>
							<div class="stock-status stock-status--out" id="<?= $mainId ?>_stock_status_out"
								style="display: none;">
								Нет в наличии
							</div>
						</div>
									...
									<div class="mb-3">
										...
										if ($arParams['SHOW_OLD_PRICE'] === 'Y') {
											...
											<div class="product-item-detail-economy-price mb-1"
												id="<?= $itemIds['DISCOUNT_PRICE_ID'] ?>" <?= ($showDiscount ? '' : 'style="display: none;"') ?>><?php
														if ($showDiscount) {
								...
								case 'buttons':
									...
									<div data-entity="main-button-container" class="mb-3">
										<div id="<?= $itemIds['BASKET_ACTIONS_ID'] ?>"
											...
											<!-- УМНЫЙ БЛОК: Селектор количества (показывается, если товар в корзине) -->
											<div class="product-item-detail-quantity-container" data-entity="quantity-block-in-cart"
												style="display: none;">
												<div class="product-quantity-container-in">
													<div class="product-quantity">
														<button class="product-quantity__button product-quantity__button--minus"
															type="button" id="<?= $itemIds['QUANTITY_DOWN_ID'] ?>">&minus;</button>
														<input class="product-quantity__input" type="number" value="1"
															id="<?= $itemIds['QUANTITY_ID'] ?>">
														<button class="product-quantity__button product-quantity__button--plus"
															type="button" id="<?= $itemIds['QUANTITY_UP_ID'] ?>">&plus;</button>
													</div>
													<!-- НОВЫЙ БЛОК ДЛЯ ССЫЛКИ И ОБЩЕЙ ЦЕНЫ -->
													<div class="product-quantity-info">
														<div class="product-quantity-total-price"
															id="<?= $itemIds['PRICE_TOTAL'] ?>">
														</div>
													</div>
												</div>
												<a href="<?= $arParams['BASKET_URL'] ?>"
													class="product-quantity__link-to-cart">Перейти
													в корзину</a>
											</div>

											<!-- УМНЫЙ БЛОК: Кнопка "В корзину" (показывается, если товара нет в корзине) -->
											<?php if ($showAddBtn) { ?>
												<div class="product-item-detail-button-container" data-entity="add-to-cart-button">
													<!-- ИЗМЕНЕНО: Добавляем классы для красивой кнопки -->
													<a class="btn btn-primary btn-lg product-item-detail-buy-button"
														id="<?= $itemIds['ADD_BASKET_LINK'] ?>" href="javascript:void(0);">
														<i
															class="fas fa-shopping-cart mr-2"></i><?= $arParams['MESS_BTN_ADD_TO_BASKET'] ?>
													</a>
												</div>
											<?php } ?>
									...
									<div class="mb-3" id="<?= $itemIds['NOT_AVAILABLE_MESS'] ?>"
										style="display: <?= (!$actualItem['CAN_BUY'] ? '' : 'none') ?>;">
										<a class="btn btn-primary product-item-detail-buy-button not-available"
											href="javascript:void(0)" rel="nofollow"><?= $arParams['MESS_NOT_AVAILABLE'] ?></a>
									</div>
									<?php
									break;
							}
...
$ingredients = $arResult["PROPERTIES"]["INGREDIENTS"];
...
<div class="p-2 product-item-detail-short-card-fixed d-none d-md-block" id="<?= $itemIds['SMALL_CARD_PANEL_ID'] ?>">
	<div class="product-item-detail-short-card-content-container">
		...
		<div class="product-item-detail-short-title-container" data-entity="panel-title">
			<div class="product-item-detail-short-title-text" title="<?= $name ?>"><?= $name ?></div>
			<?php if ($haveOffers): ?>
				<div>
		...
		<!-- Buttons / Smart Cart Logic -->
		<div class="product-item-detail-short-card-btn">

			<!-- 1. Quantity Selector (Если товар В КОРЗИНЕ) -->
			<div class="product-item-detail-quantity-container" data-entity="panel-quantity-block-in-cart"
				style="display: none;">
				<div class="product-quantity">
					<button class="product-quantity__button" type="button"
						id="<?= $itemIds['SMALL_CARD_QUANTITY_DOWN_ID'] ?>">&minus;</button>
					<input class="product-quantity__input" type="number" value="1"
						id="<?= $itemIds['SMALL_CARD_QUANTITY_ID'] ?>">
					<button class="product-quantity__button" type="button"
						id="<?= $itemIds['SMALL_CARD_QUANTITY_UP_ID'] ?>">&plus;</button>
				</div>

				<!-- ВЕРНУЛИ ИКОНКУ -->
				<a href="<?= $arParams['BASKET_URL'] ?>" class="product-quantity__link-to-cart"
					title="Перейти в корзину">
					<i class="fas fa-shopping-cart"></i>
				</a>
			</div>

			<!-- 2. Add to Cart Button (Если товара НЕТ В КОРЗИНЕ) -->
			<div data-entity="panel-add-button" style="display: <?= ($actualItem['CAN_BUY'] ? '' : 'none') ?>;">
				<?php if ($showAddBtn): ?>
					<!-- Синяя кнопка "В корзину" -->
					<a class="btn btn-primary product-item-detail-buy-button" id="<?= $itemIds['SMALL_CARD_ADD_BTN_ID'] ?>"
						href="javascript:void(0);">
						<i class="fas fa-shopping-cart mr-2"></i> <?= $arParams['MESS_BTN_ADD_TO_BASKET'] ?>
					</a>
				<?php endif; ?>

				<?php if ($showBuyBtn): ?>
					<a class="btn <?= $buyButtonClassName ?> product-item-detail-buy-button"
						id="<?= $itemIds['BUY_LINK'] ?>_panel" href="javascript:void(0);">
						<?= $arParams['MESS_BTN_BUY'] ?>
					</a>
				<?php endif; ?>
			</div>
			...
			<div style="display: <?= (!$actualItem['CAN_BUY'] ? '' : 'none') ?>;"
				...
				<span class="btn btn-link product-item-detail-buy-button disabled">
					<?= $arParams['MESS_NOT_AVAILABLE'] ?>
				</span>
			</div>
...
<script>
	BX.ready(function () {
		...
		if (obj) {
			...
			// --- Основной блок ---
			obj.visual.PAY_BLOCK = obj.obProduct.querySelector('.product-item-detail-pay-block');
			obj.visual.QUANTITY_IN_CART_BLOCK = obj.getEntity(obj.obProduct, 'quantity-block-in-cart');
			obj.visual.ADD_TO_CART_BUTTON = obj.getEntity(obj.obProduct, 'add-to-cart-button');
			obj.visual.STOCK_STATUS_IN = BX('<?= $mainId ?>_stock_status_in');
			obj.visual.STOCK_STATUS_OUT = BX('<?= $mainId ?>_stock_status_out');
			obj.obPrice.total = BX('<?= $itemIds['PRICE_TOTAL'] ?>');

			// --- Липкая панель (Small Card) ---
			if (obj.smallCardNodes.panel) {
				obj.visual.PANEL_QUANTITY_BLOCK = obj.getEntity(obj.smallCardNodes.panel, 'panel-quantity-block-in-cart');
				obj.visual.PANEL_ADD_BTN_BLOCK = obj.getEntity(obj.smallCardNodes.panel, 'panel-add-button');
				// Инпуты панели
				obj.visual.PANEL_QUANTITY_INPUT = BX('<?= $itemIds['SMALL_CARD_QUANTITY_ID'] ?>');
				obj.visual.PANEL_QUANT_UP = BX('<?= $itemIds['SMALL_CARD_QUANTITY_UP_ID'] ?>');
				obj.visual.PANEL_QUANT_DOWN = BX('<?= $itemIds['SMALL_CARD_QUANTITY_DOWN_ID'] ?>');
			}

			obj.updateCartimer = null;

			// ==============================================================
			// 2. МЕТОДЫ
			// ==============================================================

			// --- Переключение "В наличии" / "Нет в наличии" ---
			obj.updateStockStatus = function () {
				var canBuy = false;
				if (this.productType === 3) {
					if (this.offers[this.offerNum]) { canBuy = this.offers[this.offerNum].CAN_BUY; }
				} else {
					canBuy = this.product.canBuy;
				}
				if (this.visual.STOCK_STATUS_IN && this.visual.STOCK_STATUS_OUT) {
					this.visual.STOCK_STATUS_IN.style.display = canBuy ? 'flex' : 'none';
					this.visual.STOCK_STATUS_OUT.style.display = canBuy ? 'none' : 'flex';
				}
			};

			// --- Управление прозрачностью (чтобы не дергалось при загрузке) ---
			obj.toggleLoadingState = function (isLoading) {
				var elements = [
					this.visual.ADD_TO_CART_BUTTON,
					this.visual.QUANTITY_IN_CART_BLOCK,
					this.visual.PANEL_ADD_BTN_BLOCK,
					this.visual.PANEL_QUANTITY_BLOCK
				];

				elements.forEach(function (el) {
					if (el) {
						if (isLoading) el.classList.add('basket-action-loading');
						else el.classList.remove('basket-action-loading');
					}
				});
			};

			// --- Основная проверка корзины (AJAX) ---
			obj.updateBasketControls = function () {
				var productId = this.product.id;
				if (this.productType === 3) { productId = this.offers[this.offerNum].ID; }
				...
				BX.ajax({
					...
					onsuccess: BX.proxy(function (response) {
						// Убираем "заморозку" интерфейса
						this.toggleLoadingState(false);

						if (response.status !== 'success') {
							console.error('Error from get_basket.php:', response.message);
							return;
						}

						var basketItems = response.data || [];
						var inCart = false;
						var cartQuantity = 0;

						// Ищем текущий ID товара в корзине
						if (basketItems.length > 0) {
							for (var i = 0; i < basketItems.length; i++) {
								if (parseInt(basketItems[i].PRODUCT_ID) === parseInt(productId)) {
									inCart = true;
									cartQuantity = parseFloat(basketItems[i].QUANTITY);
									break;
								}
							}
						}

						// --- Обновляем видимость блоков (Основной блок) ---
						if (this.visual.QUANTITY_IN_CART_BLOCK && this.visual.ADD_TO_CART_BUTTON) {
							this.visual.ADD_TO_CART_BUTTON.style.display = inCart ? 'none' : 'block';
							this.visual.QUANTITY_IN_CART_BLOCK.style.display = inCart ? 'flex' : 'none';
						}

						// --- Обновляем видимость блоков (Липкая панель) ---
						if (this.visual.PANEL_QUANTITY_BLOCK && this.visual.PANEL_ADD_BTN_BLOCK) {
							this.visual.PANEL_ADD_BTN_BLOCK.style.display = inCart ? 'none' : 'block';
							this.visual.PANEL_QUANTITY_BLOCK.style.display = inCart ? 'flex' : 'none';
						}

						// --- Обновляем цифры в инпутах ---
						if (inCart) {
							// Если в корзине - ставим актуальное число
							this.obQuantity.value = cartQuantity;
							if (this.visual.PANEL_QUANTITY_INPUT) {
								this.visual.PANEL_QUANTITY_INPUT.value = cartQuantity;
							}
						} else {
							// Если нет - ставим 1 (или шаг товара)
							var defaultQty = this.stepQuantity || 1;
							this.obQuantity.value = defaultQty;
							if (this.visual.PANEL_QUANTITY_INPUT) {
								this.visual.PANEL_QUANTITY_INPUT.value = defaultQty;
							}
						}

						// Пересчитываем итоговую цену и состояние кнопок +/- (disabled)
						this.setPrice();
						this.checkQuantityControls();

						// Убираем спиннер загрузки с блока цены, если он был
						if (this.visual.PAY_BLOCK) {
							this.visual.PAY_BLOCK.classList.remove('is-loading');
						}
			...
			// --- Отправка данных в корзину (с задержкой) ---
			obj.sendQuantityToBasket = function (quantity) {
				var productId = this.product.id;
				if (this.productType === 3) { productId = this.offers[this.offerNum].ID; }
				...
				BX.ajax({
					...
					method: 'POST',
					dataType: 'json',
					data: { sessid: BX.bitrix_sessid(), productId: productId, quantity: quantity },
					onsuccess: function (response) {
						if (response.status === 'success') {
							BX.onCustomEvent('OnBasketChange');
						} else {
							console.error('Failed to update basket:', response.message);
						}
					}
				});
			};

			obj.debounceSendQuantity = function (val) {
				clearTimeout(this.updateCartimer);
				this.updateCartimer = setTimeout(BX.proxy(function () {
					this.sendQuantityToBasket(val);
				}, this), 600);
			};

			// ==============================================================
			// 3. ПРИВЯЗКА СОБЫТИЙ (BINDINGS)
			// ==============================================================

			// --- ОСНОВНОЙ БЛОК ---

			BX.unbindAll(obj.obQuantityUp);
			BX.bind(obj.obQuantityUp, 'click', BX.proxy(function () {
				this.quantityUp();
				this.setPrice();
				// Синхронизируем инпут панели
				if (this.visual.PANEL_QUANTITY_INPUT) this.visual.PANEL_QUANTITY_INPUT.value = this.obQuantity.value;
				this.debounceSendQuantity(this.obQuantity.value);
			}, obj));

			BX.unbindAll(obj.obQuantityDown);
			BX.bind(obj.obQuantityDown, 'click', BX.proxy(function () {
				this.quantityDown();
				this.setPrice();
				// Синхронизируем инпут панели
				if (this.visual.PANEL_QUANTITY_INPUT) this.visual.PANEL_QUANTITY_INPUT.value = this.obQuantity.value;
			...
			BX.unbindAll(obj.obQuantity);
			BX.bind(obj.obQuantity, 'change', BX.proxy(function () {
				this.quantityChange();
				this.setPrice();
				// Синхронизируем инпут панели
				if (this.visual.PANEL_QUANTITY_INPUT) this.visual.PANEL_QUANTITY_INPUT.value = this.obQuantity.value;
			...
			// ВАЖНО: Кнопку "В корзину" в панели НЕ трогаем! 
			// Пусть её обрабатывает стандартный script.js (catalog.element), иначе будет 2 товара.

			if (obj.visual.PANEL_QUANT_UP) {
				BX.bind(obj.visual.PANEL_QUANT_UP, 'click', BX.proxy(function () {
					this.quantityUp(); // Меняем в основном объекте
					this.setPrice();
					if (this.visual.PANEL_QUANTITY_INPUT) this.visual.PANEL_QUANTITY_INPUT.value = this.obQuantity.value;
					this.debounceSendQuantity(this.obQuantity.value);
				}, obj));
			...
			if (obj.visual.PANEL_QUANT_DOWN) {
				BX.bind(obj.visual.PANEL_QUANT_DOWN, 'click', BX.proxy(function () {
					this.quantityDown();
					this.setPrice();
					if (this.visual.PANEL_QUANTITY_INPUT) this.visual.PANEL_QUANTITY_INPUT.value = this.obQuantity.value;
					this.debounceSendQuantity(this.obQuantity.value);
			...
			if (obj.visual.PANEL_QUANTITY_INPUT) {
				BX.bind(obj.visual.PANEL_QUANTITY_INPUT, 'change', BX.proxy(function () {
					// Если поменяли руками в панели -> меняем в основном инпуте
					this.obQuantity.value = this.visual.PANEL_QUANTITY_INPUT.value;
					this.quantityChange();
					this.setPrice();
					this.debounceSendQuantity(this.obQuantity.value);
				}, obj));
			...
			// Подменяем стандартную обработку корзины на нашу умную
			var originalBasketResult = BX.proxy(obj.basketResult, obj);
			obj.basketResult = function (arResult) {
				originalBasketResult(arResult);
				this.updateBasketControls();
			};

			// При любом изменении корзины - обновляем кнопки
			BX.addCustomEvent('OnBasketChange', BX.proxy(obj.updateBasketControls, obj));

			// ПЛАВНАЯ СМЕНА ТОРГОВОГО ПРЕДЛОЖЕНИЯ (SKU)
			BX.addCustomEvent('onCatalogElementChangeOffer', BX.proxy(function () {
				// 1. Включаем "прозрачность", блокируем клики (нет дергания интерфейса)
				this.toggleLoadingState(true);

				// 2. Визуально сбрасываем цифру на 1 (дефолт), чтобы цена пересчиталась за 1 шт.
				var defaultQty = this.stepQuantity || 1;
				this.obQuantity.value = defaultQty;
				if (this.visual.PANEL_QUANTITY_INPUT) {
					this.visual.PANEL_QUANTITY_INPUT.value = defaultQty;
				...
				// 3. Обновляем цену (мгновенно на клиенте)
				this.setPrice();

				// 4. Запускаем AJAX. Интерфейс сменится только когда придет точный ответ.
				this.updateBasketControls();
			...
			setTimeout(BX.proxy(function () {
				this.updateBasketControls();
				this.updateStockStatus();
				// Принудительно снимаем спиннер с SKU-блока, если он там завис
				if (this.obTree) {
					this.obTree.classList.remove('is-loading');
				}
			}, obj), 50);
		}
	});
...
// echo "<pre>";
// print_r($arResult['SKU_PROPS']["WEIGHT_REFERENCE"]["VALUES"]);
// echo "<pre>";
