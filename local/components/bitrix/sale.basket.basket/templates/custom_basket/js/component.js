	...
	BX.Sale.BasketComponent = {
		...
		fillTotalBlocks: function () {
			...
			if (totalNodes && totalNodes.length) {
				...
				if (totalTemplate) {
					// Создаем новый объект данных для шаблона
					var totalData = BX.clone(this.result.TOTAL_RENDER_DATA);

					// Добавляем недостающие свойства, которые нужны в новом шаблоне
					totalData.BASKET_ITEMS_COUNT = this.result.BASKET_ITEMS_COUNT;
					totalData.GOODS_MESSAGE = this.getGoodsMessage(this.result.BASKET_ITEMS_COUNT);

					// Передаем в рендер уже обогащенный объект
					var totalRender = this.render(totalTemplate, totalData);
		...
		bindActionEvents: function (node, data) {
			...
			// ---> НАЧАЛО: НОВЫЙ КОД ДЛЯ ЧЕКБОКСОВ <---
			var checkbox = this.getEntity(node, 'basket-item-select');
			if (checkbox) {
				BX.bind(checkbox, 'change', BX.proxy(this.toggleItemDelay, this));
			}
			// ---> КОНЕЦ: НОВЫЙ КОД ДЛЯ ЧЕКБОКСОВ <---
		},

		/**
	* ИСПРАВЛЕНО: Функция теперь находит ОБА чекбокса "Выбрать все" (для десктопа и мобильных)
	* и синхронно обновляет их состояние.
	* Она больше не вызывает никаких событий и не блокирует чекбоксы без необходимости.
	*/
		updateSelectAllCheckbox: function () {
			// Находим ОБА главных чекбокса по их data-entity
			var selectAllCheckboxes = document.querySelectorAll('[data-entity="basket-select-all-items"]');
			if (selectAllCheckboxes.length === 0) return;

			// Находим все чекбоксы товаров
			var allItemCheckboxes = document.querySelectorAll('[data-entity="basket-item-select"]');
			var allChecked = true;
			var isDisabled = allItemCheckboxes.length === 0;

			// Если товаров нет, выходим сразу
			if (isDisabled) {
				allChecked = false;
			} else {
				// Проверяем, выбран ли каждый товар
				for (var i = 0; i < allItemCheckboxes.length; i++) {
					if (!allItemCheckboxes[i].checked) {
						allChecked = false; // Если хотя бы один не выбран, то и "Выбрать все" не выбрано
						break;
					}
				}
			}

			// Применяем состояние ко ВСЕМ найденным чекбоксам "Выбрать все"
			for (var j = 0; j < selectAllCheckboxes.length; j++) {
				selectAllCheckboxes[j].checked = allChecked;
				selectAllCheckboxes[j].disabled = isDisabled;
			}
		},


		/**
		 * ИСПРАВЛЕНО: Логика обработки клика по чекбоксу одного товара.
		 * Теперь она надежно работает в связке с остальными функциями.
		 */
		toggleItemDelay: function (event) {
			var target = BX.getEventTarget(event);
			var itemData = this.getItemDataByTarget(target);
			var itemNode = BX(this.ids.item + itemData.ID);

			if (!itemData || !itemNode) return;

			// В зависимости от состояния чекбокса, добавляем или убираем товар из отложенных
			if (target.checked) {
				// Если выбрали (поставили галочку), товар должен быть активен
				BX.removeClass(itemNode, 'delayed');
				this.actionPool.removeDelayed(itemData.ID);
			} else {
				// Если галочку убрали, товар откладывается
				BX.addClass(itemNode, 'delayed');
				this.actionPool.addDelayed(itemData.ID);
			}

			// После изменения одного элемента, обновляем состояние чекбокса "Выбрать все"
			this.updateSelectAllCheckbox();
		},

		/**
		* ИСПРАВЛЕНО: Полностью переписанная логика для чекбокса "Выбрать все".
		* Навешивает один обработчик на оба чекбокса и синхронизирует их.
		*/
		bindHeaderActions: function () {
			// Находим оба главных чекбокса по data-entity
			var selectAllCheckboxes = document.querySelectorAll('[data-entity="basket-select-all-items"]');
			if (selectAllCheckboxes.length > 0) {
				var self = this; // Сохраняем контекст

				// Создаем обработчик
				var selectAllHandler = function (event) {
					var isChecked = event.target.checked;
					var allItemCheckboxes = document.querySelectorAll('[data-entity="basket-item-select"]');

					// 1. Синхронизируем состояние всех чекбоксов "Выбрать все"
					for (var k = 0; k < selectAllCheckboxes.length; k++) {
						if (selectAllCheckboxes[k] !== event.target) {
							selectAllCheckboxes[k].checked = isChecked;
						}
					}

					// 2. Проходим по всем чекбоксам товаров
					for (var i = 0; i < allItemCheckboxes.length; i++) {
						var currentCheckbox = allItemCheckboxes[i];

						// Меняем состояние только тех чекбоксов, которые отличаются
						if (currentCheckbox.checked !== isChecked) {
							currentCheckbox.checked = isChecked;

							// Получаем данные товара и напрямую меняем его статус в actionPool
							var itemData = self.getItemDataByTarget(currentCheckbox);
							var itemNode = BX(self.ids.item + itemData.ID);

							if (isChecked) {
								BX.removeClass(itemNode, 'delayed');
								self.actionPool.removeDelayed(itemData.ID);
							} else {
								BX.addClass(itemNode, 'delayed');
								self.actionPool.addDelayed(itemData.ID);
							}
						}
					}
				};

				// Навешиваем обработчик на каждый найденный чекбокс
				for (var j = 0; j < selectAllCheckboxes.length; j++) {
					BX.bind(selectAllCheckboxes[j], 'change', selectAllHandler);
				}
			}

			// Первоначальная проверка состояния "Выбрать все" при загрузке страницы
			this.updateSelectAllCheckbox();
		},
