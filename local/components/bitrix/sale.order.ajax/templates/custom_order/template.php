...
if ((string) $request->get('ORDER_ID') !== '') {
	...
	?>
	<form action="<?= POST_FORM_ACTION_URI ?>" method="POST" name="ORDER_FORM" class="bx-soa-wrapper mb-4<?= $themeClass ?>"
		...
		<div id="bx-soa-order" class="row" style="opacity: 0">
			...
			<div class="col-lg-8 bx-soa">
				...
				<?php // >>> НАЧАЛО ИЗМЕНЕНИЙ: Блок свойств покупателя перенесен на первое место <<< ?>
				<!--	BUYER PROPS BLOCK	-->
				<div id="bx-soa-properties" data-visited="false" class="bx-soa-section bx-active">
					<div
						class="bx-soa-section-title-container d-flex justify-content-between align-items-center flex-nowrap">
						<div class="bx-soa-section-title" data-entity="section-title">
				...
				<?php // >>> НАЧАЛО ИЗМЕНЕНИЙ: Старый блок корзины полностью удален <<< ?>
				<?php // BASKET ITEMS BLOCK IS REMOVED ?>
				<?php // >>> КОНЕЦ ИЗМЕНЕНИЙ <<< ?>


				<!-- REGION BLOCK IS REMOVED FROM HERE -->
				...
				if ($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d'):
					...
					<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active" <?= ($hideDelivery ? 'style="display:none"' : '') ?>>
						<div
							...
							<div><a href="" class="bx-soa-editstep" style="display: none;"><?= $arParams['MESS_EDIT'] ?></a>
							</div>
						</div>
						<div class="bx-soa-section-content"></div>
					</div>
					<!--	PICKUP BLOCK IS REMOVED FROM HERE	-->
					<?php
				else:
					?>
					<!--	UNIFIED DELIVERY BLOCK	-->
				...
				<!-- <div id="bx-soa-orderSave">
					<a href="javascript:void(0)" style="margin: 10px 0"
						class="btn btn-primary btn-lg d-none d-sm-inline-block js-order-save-button">
						<?= $arParams['MESS_ORDER'] ?>
					</a>
				</div> -->
			...
			<!--	SIDEBAR BLOCK	-->
			<div id="bx-soa-total" class="col-lg-4 bx-soa-sidebar">
				<div class="bx-soa-cart-total-ghost"></div>
				<div class="bx-soa-cart-total">
					<div class="bx-soa-cart-total-main">
						<div id="bx-soa-total-lines">
							<!-- Сюда JS будет вставлять итоги, купоны и кнопку -->
						</div>
						<div class="bx-soa-sidebar-basket">
							<div class="bx-soa-sidebar-basket-title"><?= $arParams['MESS_BASKET_BLOCK_NAME'] ?></div>
							<div id="bx-soa-sidebar-basket-items">
								<!-- Сюда JS будет вставлять товары -->
							</div>
						</div>
					</div>
					<?php if ($arParams['USER_CONSENT'] === 'Y'): ?>
						<div class="checkbox" style="padding: 15px 18px 0 18px;">
							<div class="form-check">
								<input class="form-check-input" type="checkbox" value="Y" id="soa-user-consent"
									name="USER_CONSENT" checked>
								<label class="form-check-label" for="soa-user-consent">
									<span class="form-checbox__text">Продолжая, вы соглашаетесь с <a href="/privacy-policy/"
											target="_blank">политикой конфиденциальности</a></span>
								</label>
								<div id="soa-user-consent-error" class="soa-field-error-message"></div>
							</div>
						</div>
					<?php endif; ?>
				</div>
	...
	// >>> НАЧАЛО ИЗМЕНЕНИЙ: Определяем свойства для переноса <<<
	$propsToDelivery = ['ADDRESS']; // Укажите здесь символьный код вашего свойства "Адрес доставки"
	foreach ($arResult['JS_DATA']['ORDER_PROP']['properties'] as $prop) {
		if ($prop['IS_ZIP'] == 'Y') {
			$propsToDelivery[] = $prop['CODE'];
			break;
		}
	}
	// >>> КОНЕЦ ИЗМЕНЕНИЙ <<<
	...
	<script>
		...
		BX.Sale.OrderAjaxComponent.init({
			...
			// >>> НАЧАЛО ИЗМЕНЕНИЙ: Передаем коды свойств в JS <<<
			propsToDelivery: <?= CUtil::PhpToJSObject($propsToDelivery) ?>,
			// >>> КОНЕЦ ИЗМЕНЕНИЙ <<<
	...
	if ($arParams['SHOW_PICKUP_MAP'] === 'Y' || $arParams['SHOW_MAP_IN_PROPS'] === 'Y') {
		if ($arParams['PICKUP_MAP_TYPE'] === 'yandex') {
			...
			?>
			<script
				src="<?= $scheme ?>://api-maps.yandex.ru/2.1.50/?apikey=<?= $apiKey ?>&load=package.full&lang=<?= $locale ?>"></script>
			<script>
