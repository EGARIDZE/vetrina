...
(function () {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function (parameters) {
			...
			// console.log("DEBUG INIT: Полные параметры, пришедшие в компонент:", JSON.parse(JSON.stringify(parameters)));
			this.initializePrimaryFields();

			this.constPropsToDelivery = parameters.propsToDelivery || [];

			this.propsToDelivery = parameters.propsToDelivery || [];
			...
			this.mobileTotalBlockNode = BX(parameters.totalBlockId + '-mobile') || { style: {}, querySelector: function () { return null; } };
			this.savedFilesBlockNode = BX('bx-soa-saved-files');
			this.orderSaveBlockNode = BX('bx-soa-orderSave') || { style: {} };
			...
			// >>> НАЧАЛО ИСПРАВЛЕНИЯ <<<
			// Создаем безопасные "заглушки" для удаленных блоков.
			// Это гарантирует, что любые обращения к .id, .style или другим свойствам не вызовут ошибок.
			this.basketBlockNode = BX(parameters.basketBlockId) || { id: 'bx-soa-basket-dummy', style: {}, querySelector: function () { return null; }, querySelectorAll: function () { return []; } };
			this.basketHiddenBlockNode = BX(parameters.basketBlockId + '-hidden') || { id: 'bx-soa-basket-hidden-dummy', style: {} };
			this.regionBlockNode = { id: 'bx-soa-region-dummy', style: {}, getAttribute: function () { return '' }, setAttribute: function () { } };
			this.regionHiddenBlockNode = { id: 'bx-soa-region-hidden-dummy', style: {} };
			this.pickUpBlockNode = { id: 'bx-soa-pickup-dummy', style: {}, querySelector: function () { return null; }, getAttribute: function () { return '' }, setAttribute: function () { } };
			this.pickUpHiddenBlockNode = { id: 'bx-soa-pickup-hidden-dummy', style: {} };
			// >>> КОНЕЦ ИСПРАВЛЕНИЯ <<<
		...
		refreshOrder: function (result) {
			if (result.error) {
				...
				this.result = result.order; // <--- Данные обновились
				this.prepareLocations(result.locations);

				this.propsToDelivery = this.constPropsToDelivery;
				...
				// >>> НАЧАЛО ИСПРАВЛЕНИЯ <<<
				// После отрисовки HTML, инициализируем сложные контроли.
				// Это правильное место для AJAX-обновлений.
		...
		checkNotifications: function () {
			...
			if (informer) {
				if (this.firstLoad && this.result.IS_AUTHORIZED && typeof this.result.LAST_ORDER_DATA.FAIL === 'undefined') {
					...
					// >>> НАЧАЛО ИСПРАВЛЕНИЯ <<<
					// Очищаем контейнер от всех предыдущих сообщений перед добавлением нового.
		...
		checkPreload: function (node) {
			// Если узел не является валидным DOM-элементом или это наш "фантомный" блок,
			// считаем, что предварительная загрузка не требуется.
			if (!node || !node.id || node.id === this.regionBlockNode.id || node.id === this.pickUpBlockNode.id) {
				return true;
			}
			...
			switch (node.id) {
				// case this.regionBlockNode.id: // Эти case больше не нужны
				...
				case this.deliveryBlockNode.id:
					...
					break;
				// case this.pickUpBlockNode.id: // И этот
		...
		bindEvents: function () {
			// Мы убрали привязку к удаленной кнопке [data-save-button]
		...
		initPickUpPagination: function () {
			...
			var currentDelivery = this.getSelectedDelivery(); // Получаем актуальную доставку

			// Если есть доставка, у нее есть ПВЗ (STORE), и их больше 0
			if (currentDelivery && currentDelivery.STORE && currentDelivery.STORE.length > 0) {
				stores = this.getPickUpInfoArray(currentDelivery.STORE);
				usePickUpPagination = stores.length > this.options.pickUpsPerPage;
			} else {
				// Если ПВЗ нет, обнуляем пагинацию и выходим
				this.pickUpPagination = {};
				return;
			}

			if (stores && stores.length > 0) {
				// Логика пагинации (остается почти без изменений)
				if (this.options.pickUpsPerPage > 0 && usePickUpPagination) {
					var arReserve = stores.slice();
					var pages = Math.ceil(arReserve.length / this.options.pickUpsPerPage);
					var arPages = [];
				...
				else { // <-- Если пагинация не нужна (ПВЗ мало)
					this.pickUpPagination.pageNumber = 1;
					this.pickUpPagination.currentPage = stores; // <-- Просто записываем все ПВЗ в currentPage
					this.pickUpPagination.show = false;
				}
			} else {
				// На всякий случай еще раз обнуляем, если stores оказались пустыми
				this.pickUpPagination = {};
			}
		},
		...
		debugLocations: function () {
			console.log('--- ЗАПУСК ДИАГНОСТИКИ МЕСТОПОЛОЖЕНИЙ ---');
			// Используем селектор, который найдет поле и в десктопной, и в мобильной версии
			var universalSelector = '.bx-ui-sls-fake, .dropdown-field';

			var locationInputContainer = BX('soa-location-input-container');
			console.log('1. Поиск контейнера #soa-location-input-container:', locationInputContainer ? 'НАЙДЕН' : 'НЕ НАЙДЕН');

			if (locationInputContainer) {
				var fakeInput = locationInputContainer.querySelector(universalSelector);
				console.log('2. Поиск поля ввода по селектору "' + universalSelector + '":', fakeInput ? 'НАЙДЕНО' : 'НЕ НАЙДЕНО');

				if (fakeInput) {
					console.log('3. Атрибут title поля ввода:', '"' + fakeInput.title + '"');
					console.log('4. Текущее значение (value) поля ввода:', '"' + fakeInput.value + '"');

					if (fakeInput.title) {
						var cityName = fakeInput.title.split(',')[0];
						console.log('5. Результат парсинга title (название города):', '"' + cityName + '"');
					} else {
						console.log('5. Парсинг title НЕВОЗМОЖЕН, так как title пустой. (Это может быть нормально на начальном этапе загрузки)');
					}
				}
			}
		...
		locationsCompletion: function () {
			...
			if (this.deliveryBlockNode && this.deliveryHiddenBlockNode && BX.type.isDomNode(this.deliveryBlockNode)) {
				this.fixLocationsStyle(this.deliveryBlockNode, this.deliveryHiddenBlockNode);
			}
			if (this.propsBlockNode && this.propsHiddenBlockNode && BX.type.isDomNode(this.propsBlockNode)) {
				this.fixLocationsStyle(this.propsBlockNode, this.propsHiddenBlockNode);
			}

			// --- НАЧАЛО ИСПРАВЛЕНИЯ С ЗАДЕРЖКОЙ ---
			// Даем системным скриптам Битрикса ~100мс на то, чтобы "оживить" компонент и добавить title.
			// Это делает исправление более надежным на медленных устройствах.
			setTimeout(function () {
				var locationInputContainer = BX('soa-location-input-container');
				if (locationInputContainer) {
					var fakeInput = locationInputContainer.querySelector('.bx-ui-sls-fake, .dropdown-field');
					var cityNameSpan = BX('soa-location-city-name');

					if (fakeInput && fakeInput.title) {
						var cityName = fakeInput.title.split(',')[0];

						if (cityNameSpan) {
							cityNameSpan.innerHTML = BX.util.htmlspecialchars(cityName);
						}

						// Если в поле все еще код, заменяем его на название.
						if (!isNaN(parseInt(fakeInput.value))) {
							fakeInput.value = cityName;
						}
					}
				}
			}, 100);
		...
		fixLocationsStyle: function (section, hiddenSection) {
			...
			// Применяем класс form-control к пошаговым инпутам
		...
		clickOrderSaveAction: function (event) {
			// Всегда предотвращаем стандартное действие, чтобы взять управление на себя
			BX.PreventDefault(event);

			var consentCheckbox = BX('soa-user-consent');
			var consentErrorNode = BX('soa-user-consent-error');

			// Если чекбокс найден и он НЕ отмечен
			if (consentCheckbox && !consentCheckbox.checked) {
				if (consentErrorNode) {
					consentErrorNode.innerHTML = BX.message('SOA_ERROR_CONSENT');
					setTimeout(function () { BX.addClass(consentErrorNode, 'active'); }, 10);
				}
				this.animateScrollTo(BX('bx-soa-total'));

				// Выходим из функции, дальнейший код не выполнится
				return;
			}

			// Если проверка пройдена, убираем ошибку (если она была)
			if (consentErrorNode) {
				BX.removeClass(consentErrorNode, 'active');
			}

			// Запускаем валидацию остальных полей и отправку заказа
			if (this.isValidForm()) {
				this.allowOrderSave();
				this.doSaveAction();
			}
		...
		clickNextAction: function (event) {
			// Пусто. Предотвращаем стандартное действие.
			return BX.PreventDefault(event);
		},
		...
		showByClick: function (event) {
			var target = event.target || event.srcElement;
			// Ищем родительский блок-секцию
			var sectionNode = BX.findParent(target, { className: "bx-soa-section" });

			if (sectionNode) {
				var content = sectionNode.querySelector('.bx-soa-section-content');
				var editLink = sectionNode.querySelector('.bx-soa-editstep');

				// Если блок свернут (имеет класс bx-step-completed)
				if (BX.hasClass(sectionNode, 'bx-step-completed')) {
					// Разворачиваем его
					BX.removeClass(sectionNode, 'bx-step-completed');
					BX.addClass(sectionNode, 'bx-selected');
					if (editLink) editLink.style.display = 'none';
					if (content) content.style.display = '';
				}
				else // Иначе, если блок развернут
				{
					// Сворачиваем его
					BX.removeClass(sectionNode, 'bx-selected');
					BX.addClass(sectionNode, 'bx-step-completed');
		...
		showActualBlock: function () {
			...
			while (allSections[i]) {
				...
				// Явно пропускаем обработку секций, которые мы логически удалили.
				if (section.id === 'bx-soa-region' || section.id === 'bx-soa-pickup') {
					i++;
					continue;
				}

				if (section.id === (this.propsBlockNode && this.propsBlockNode.id))
					this.isValidPropertiesBlock();

				if (!this.checkBlockErrors(section) || !this.checkPreload(section)) {
					if (this.activeSectionId !== section.id) {
						if (BX(this.activeSectionId)) {
							this.fade(BX(this.activeSectionId));
						}
						this.show(section);
					}
					break;
		...
		changeVisibleContent: function () {
			...
			for (i = 0; i < sections.length; i++) {
				// Показываем контент блока
				var content = sections[i].querySelector('.bx-soa-section-content');
				if (content)
					content.style.display = '';

				// Скрываем ссылку "Изменить", т.к. блок и так открыт
				var editStep = sections[i].querySelector('.bx-soa-editstep');
				if (editStep)
					editStep.style.display = 'none';

				sections[i].setAttribute('data-visited', 'true');
				BX.addClass(sections[i], 'bx-selected'); // "Выбранный", т.е. развернутый
				BX.removeClass(sections[i], 'bx-step-completed'); // НЕ "завершенный", т.е. НЕ свернутый
			}

			// Кнопка "Оформить заказ" всегда видна
		...
		editOrder: function () {
			...
			for (i = 0; i < sections.length; i++) {
				...
				if (section) {
					if (section.id === 'bx-soa-region' || section.id === 'bx-soa-pickup') {
						if (BX(section.id)) {
							BX(section.id).style.display = 'none';
						}
						continue;
					}
			...
			if (this.deliveryBlockNode) {
				this.editSection(this.deliveryBlockNode);
			}

			// >>> НАЧАЛО ИСПРАВЛЕНИЯ <<<
			// Вызываем проверку отображения ПВЗ и завершение инициализации полей местоположения.
			// Это необходимо для корректной отрисовки при первоначальной загрузке страницы,
			// когда способ доставки с самовывозом может быть уже выбран.
			this.checkPickUpShow();
			this.locationsCompletion();
			// >>> КОНЕЦ ИСПРАВЛЕНИЯ <<<
		...
		editSection: function (section) {
			...
			else if (section.id != (this.pickUpBlockNode && this.pickUpBlockNode.id))
			...
			if (errorContainer) {
				this.hasErrorSection[section.id] = errorContainer.style.display != 'none';
			}
		...
		getCouponNode: function (coupon, active) {
			...
			return BX.create('STRONG', {
				...
				children: active ? [
					BX.create('SPAN', { props: { className: 'bx-soa-coupon-text' }, text: couponName || '' }),
					BX.create('SPAN', {
		...
		// --- ЗАМЕНА ФУНКЦИИ editRegionBlock ---
		editRegionBlock: function (active, container) {
			if (!this.result.PERSON_TYPE)
				return;

			// Если контейнер передан, значит, мы работаем в новой логике
			if (container) {
				this.editActiveRegionBlock(true, container);
			} else { // Иначе - работаем по старой логике (на всякий случай)
				if (active)
					this.editActiveRegionBlock(true);
				else
					this.editFadeRegionBlock();
		...
		editActiveRegionBlock: function (activeNodeMode, container) {
			var regionContent, regionNode, regionNodeCol;

			// Если контейнер не передан, функция просто ничего не будет делать.
			if (!container) return;

			BX.cleanNode(container); // Очищаем контейнер перед отрисовкой
			...
			// Эти функции отрисуют поля выбора типа плательщика, профиля и города
		...
		getDeliveryLocationInput: function (node) {
			...
			var currentProperty, locationId, location, k,
				labelHtml, currentLocation, insertedLoc;

			// --- Этап 1: Ищем свойство местоположения ---
			...
			// --- Этап 2: Создаем скрытый контейнер для поля ввода ---
			var locationInputContainer = BX.create('DIV', {
				props: { id: 'soa-location-input-container' },
				style: { display: 'none' }
			});
			...
			if (location && location[0] && location[0].output) {
				...
				// Выполняем скрипты, которые "оживляют" HTML
				...
				locationInputContainer.appendChild(insertedLoc);
			}

			this.getZipLocationInput(locationInputContainer);
			// if (location && location[0]) {
			// 	locationInputContainer.appendChild(
			// 		BX.create('DIV', { props: { className: 'bx-soa-reference' }, html: this.params.MESS_REGION_REFERENCE })
			// 	);
			// }

			// --- Этап 3: Создаем видимый блок с ПУСТЫМ местом для города ---
			var locationDisplayContainer = BX.create('DIV', { props: { id: 'soa-location-display-container', className: 'bx-soa-location-display' } });

			var displayTitle = BX.create('H2', {
				props: { className: 'bx-soa-section-title' },
				children: [
					BX.create('SPAN', { text: 'Способ доставки' }),
					BX.create('SPAN', {
						props: { className: 'city_destination' },
						// --- ВАЖНО: Создаем span с ID, который найдем позже ---
						html: ' в <span class="destination_value" id="soa-location-city-name">...</span>'
					}),
					BX.create('SPAN', {
						props: { className: 'change_city font_13 colored' },
						text: 'Другой город',
						events: {
							click: function () {
								var displayBlock = BX('soa-location-display-container');
								var inputBlock = BX('soa-location-input-container');
								if (displayBlock && inputBlock) {
									displayBlock.style.opacity = '0';
									setTimeout(function () {
										displayBlock.style.display = 'none';
										inputBlock.style.display = 'block';
										inputBlock.style.opacity = '0';
										setTimeout(function () { inputBlock.style.opacity = '1'; }, 10);
									}, 300);
								}
							}
						}
			...
			locationDisplayContainer.appendChild(displayTitle);

			// --- Этап 4: Добавляем все в DOM ---
			node.appendChild(locationDisplayContainer);
			node.appendChild(locationInputContainer);
		},

		isValidDeliveryPropertiesBlock: function () {
			if (!this.options.propertyValidation)
				return [];

			var deliveryPropsContainer = this.deliveryBlockNode.querySelector('#bx-soa-delivery-props-container');
			if (!deliveryPropsContainer)
				return [];

			var props = deliveryPropsContainer.querySelectorAll('.bx-soa-customer-field[data-property-id-row]'),
				propsErrors = [],
		...
		getZipLocationInput: function (node) {
			...
			// Ищем свойство, у которого есть системный признак "Индекс" (IS_ZIP == 'Y').
			for (i in this.result.ORDER_PROP.properties) {
			...
			// Если такое свойство найдено, просто выходим из функции.
			// Это гарантирует, что поле "Индекс" НИКОГДА не будет отрисовано здесь.
			if (zipProperty) {
				return;
			}
		},
		...
		editPaySystemItems: function (paySystemNode) {
			...
			var paySystemItemsContainer = BX.create('DIV', { props: { className: 'col-12 bx-soa-pp-item-container' } }),
		...
		createPaySystemItem: function (item) {
			...
			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
				...
				// --- НОВАЯ ЛОГИКА ---
				// Всегда показываем подсказку, если есть описание
				if (item.DESCRIPTION) {
					hintNode = BX.create('DIV', {
						props: { className: 'hint' },
						children: [
							BX.create('SPAN', {
								props: { className: 'hint__icon' },
								children: [BX.create('I', { html: '?' })]
							}),
							BX.create('DIV', {
					...
					var titleContainer = BX.create('DIV', {
						props: { className: 'bx-soa-pp-company-title-container' },
						children: [title, hintNode]
					});
					containerChildren.push(titleContainer);
				} else {
					containerChildren.push(title);
				}
			}
			...
			containerChildren.push(logoNode);
		...
		editActiveDeliveryBlock: function (activeNodeMode) {
			...
			// 1. Отрисовываем выбор региона (города)
			var regionContainer = BX.create('DIV', {
				props: { className: 'bx-soa-section-location' }
			});
			deliveryContent.appendChild(regionContainer);
			this.editRegionBlock(true, regionContainer);

			// 2. Отрисовываем список способов доставки
			var deliveryNode = BX.create('DIV', { props: { className: 'bx-soa-pp row' } });
			deliveryContent.appendChild(deliveryNode);
			this.editDeliveryItems(deliveryNode);

			// --- НАЧАЛО ИЗМЕНЕНИЙ ---
			// 3. Создаем контейнер для свойств доставки и вызываем функцию их отрисовки
			var deliveryPropsContainer = BX.create('DIV', {
				props: {
					id: 'bx-soa-delivery-props-container',
					className: 'row' // Добавляем класс row для корректной работы колонок
				}
			});
			deliveryContent.appendChild(deliveryPropsContainer);
			this.renderDeliveryProperties(deliveryPropsContainer);
			// --- КОНЕЦ ИЗМЕНЕНИЙ ---

			// 4. Создаем ПУСТОЙ контейнер для самовывоза.
			var pickUpContainer = BX.create('DIV', {
				props: { id: 'bx-soa-pickup-container' },
				style: { display: 'none' }
			});
			deliveryContent.appendChild(pickUpContainer);
		},
		...
		createDeliveryItem: function (item) {
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode, logoNode, priceNode, descriptionNode, hintNode;

			// Создаем заголовок
			if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y') {
				var titleText = this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME;
				title = BX.create('DIV', {
					props: { className: 'bx-soa-pp-company-smalltitle' },
					text: titleText
				});
				...
				// --- НОВАЯ ЛОГИКА ---
				// Если доставка НЕ активна и есть описание - показываем подсказку
				if (!checked && item.DESCRIPTION) {
					hintNode = BX.create('DIV', {
						props: { className: 'hint' },
						children: [
							BX.create('SPAN', {
					...
					labelNodes.push(titleContainer);
				} else {
					// Если доставка активна или нет описания, просто показываем заголовок
					labelNodes.push(title);
				}
			}

			// Цена (остается без изменений)
			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined' || (deliveryCached && (deliveryCached.PRICE >= 0 || typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined'))) {
				var priceHtml = '';
				if (typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined') {
					priceHtml = item.DELIVERY_DISCOUNT_PRICE_FORMATED;
				} else if (item.PRICE_FORMATED) {
					priceHtml = item.PRICE_FORMATED;
				} else if (deliveryCached) {
					priceHtml = typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined' ?
			...
			// --- НОВАЯ ЛОГИКА ---
			// Если доставка АКТИВНА и есть описание - показываем его под картинкой
			if (checked && item.DESCRIPTION) {
				descriptionNode = BX.create('DIV', {
		...
		editDeliveryItems: function (deliveryNode) {
			...
			// >>> НАЧАЛО ИСПРАВЛЕНИЯ <<<
			// Меняем класс контейнера, чтобы он занимал всю ширину
			var deliveryItemsContainer = BX.create('DIV', { props: { className: 'col-12 bx-soa-pp-item-container' } }),
				// >>> КОНЕЦ ИСПРАВЛЕНИЯ <<<
			...
			// >>> НАЧАЛО ИСПРАВЛЕНИЯ <<<
			// Просто удаляем или комментируем вызов функции
		...
		selectDelivery: function (event) {
			console.log("[1] selectDelivery: Сработал клик по доставке."); // <-- ВСТАВЬТЕ ЭТУ СТРОКУ
			if (!this.orderBlockNode) return;

			var target = event.target || event.srcElement;
			var actionSection = BX.findParent(target, { className: 'bx-soa-pp-company' });
			...
			var deliveryItemsRow = BX.findParent(actionSection, { className: 'row' });
			if (!deliveryItemsRow) return;

			var selectedSection = deliveryItemsRow.querySelector('.bx-soa-pp-company.bx-selected');
			var actionInput = actionSection.querySelector('input[type=checkbox]');

			if (selectedSection) {
				BX.removeClass(selectedSection, 'bx-selected');
				var selectedInput = selectedSection.querySelector('input[type=checkbox]');
				if (selectedInput) {
		...
		getSelectedDelivery: function () {
			var deliveryRadio = this.deliveryBlockNode.querySelector('input[type=checkbox][name=DELIVERY_ID]:checked'),
				currentDelivery = false,
				deliveryId, i;

			if (!deliveryRadio)
				deliveryRadio = this.deliveryHiddenBlockNode.querySelector('input[type=checkbox][name=DELIVERY_ID]:checked');

			if (!deliveryRadio)
				deliveryRadio = this.deliveryHiddenBlockNode.querySelector('input[type=hidden][name=DELIVERY_ID]');

			if (deliveryRadio) {
				deliveryId = deliveryRadio.value;
		...
		activatePickUp: function () {
			var pickUpBlock = BX('bx-soa-pickup');
			if (pickUpBlock) {
				pickUpBlock.style.display = '';
				this.editPickUpBlock(true, pickUpBlock);
			}
		},

		editPickUpBlock: function (active, container) {
			if (!this.result.DELIVERY) return;
			this.initialized.pickup = false;
			if (active) {
				this.editActivePickUpBlock(true, container);
			}
			this.initialized.pickup = true;
		},

		editActivePickUpBlock: function (activeNodeMode, container) {
			if (!container) return;
			BX.cleanNode(container);

			// --- НАЧАЛО ИЗМЕНЕНИЙ: Упрощаем структуру, объединяя классы ---

			// Создаем ОДИН div, который будет и контейнером, и строкой (.row)
			var mainRow = BX.create('DIV', {
				props: { className: 'bx_soa_pickup row' }
			});

			// Колонка для списка ПВЗ (без изменений)
			var listCol = BX.create('DIV', {
				props: { className: 'col-md-6 col-12 soa-pickup-list-col' },
				children: [
					BX.create('DIV', { props: { id: 'pickup-list-container' } })
				]
			});

			// Колонка для карты (без изменений)
			var mapCol = BX.create('DIV', { props: { className: 'col-md-6 d-md-block bx_soa_pickup_map' } });

			if (this.params.SHOW_PICKUP_MAP == 'Y') {
				mapCol.appendChild(BX.create('DIV', {
					props: { id: 'pickUpMap' },
					style: { width: '100%', height: '100%', minHeight: '400px', borderRadius: 'var(--border-radius-medium)' }
				}));
			}

			mainRow.appendChild(listCol);
			mainRow.appendChild(mapCol);

			// Добавляем нашу объединенную строку напрямую в главный контейнер
			container.appendChild(mainRow);

			// --- КОНЕЦ ИЗМЕНЕНИЙ ---

			this.editPickUpList(true, BX('pickup-list-container'));
		...
		// Показывает или скрывает блок самовывоза в зависимости от выбранной доставки
		checkPickUpShow: function () {
			...
			// Проверяем, что у доставки есть свойство STORE и оно не пустое
			var stores = (currentDelivery.STORE && currentDelivery.STORE.length > 0)
				? this.getPickUpInfoArray(currentDelivery.STORE)
				: [];

			if (stores.length > 0) {
				// Если массив ПВЗ не пустой, активируем блок
				this.activatePickUp();
			} else {
				// Если массив пустой, деактивируем
				this.deactivatePickUp();
			}
		},

		// Показывает блок самовывоза и запускает его отрисовку
		activatePickUp: function () {
			var pickUpContainer = BX('bx-soa-pickup-container');
			if (pickUpContainer) {
				pickUpContainer.style.display = '';
				// Запускаем отрисовку содержимого блока самовывоза, передавая ему контейнер
				this.editActivePickUpBlock(true, pickUpContainer);
			}
		},

		// Скрывает и очищает блок самовывоза
		deactivatePickUp: function () {
			var pickUpContainer = BX('bx-soa-pickup-container');
			if (pickUpContainer) {
				pickUpContainer.style.display = 'none';
				BX.cleanNode(pickUpContainer);
			}
		},
		...
		initMaps: function () {
			// Если наш объект-помощник уже создан, ничего не делаем
			if (this.maps) {
				return;
			}

			// >>> НАЧАЛО ИСПРАВЛЕНИЯ <<<
			// Проверяем, успел ли загрузиться скрипт Яндекс.Карт.
			if (typeof ymaps === 'undefined') {
				return;
			}


			// Используем официальный способ дождаться полной готовности API
			ymaps.ready(BX.delegate(function () {

				// Только теперь, когда API 100% готово, создаем наш объект-помощник
				this.maps = BX.Sale.OrderAjaxComponent.Maps.init(this);

				if (this.maps) {
					this.mapsReady = true; // Устанавливаем наш внутренний флаг
					this.resizeMapContainers();

					// ВАЖНО: Проверяем, не нужно ли нам отрисовать карту прямо сейчас.
					// Это случай, когда страница загрузилась с уже выбранным самовывозом,
					// а API карт загрузилось с небольшой задержкой.
					var pickUpContainer = BX('bx-soa-pickup-container');
					if (pickUpContainer && pickUpContainer.style.display !== 'none') {
						var pickupListContainer = BX('pickup-list-container');
						if (pickupListContainer) {
							// Перерисовываем список ПВЗ и карту
							this.editPickUpList(false, pickupListContainer);
						}
					}
				} else {
					console.error('[initMaps] Maps helper object FAILED to create.');
				}
			}, this));
			// >>> КОНЕЦ ИСПРАВЛЕНИЯ <<<
		},
		...
		editPickUpList: function (isNew, containerNode) { // <--- Принимаем переданный контейнер

			if (!containerNode) { // <--- Проверяем переданный контейнер
				console.error("   - ОШИБКА: Контейнер для списка ПВЗ не был передан!");
				return;
			}

			// Получаем актуальные данные о ПВЗ
			var currentDelivery = this.getSelectedDelivery();
			var stores = (currentDelivery && currentDelivery.STORE) ? this.getPickUpInfoArray(currentDelivery.STORE) : [];

			if (!stores.length) {
				BX.cleanNode(containerNode); // Если ПВЗ нет, очищаем контейнер (убираем лоадер)
				return;
			}

			// Инициализируем пагинацию на основе актуальных данных
			this.initPickUpPagination();

			// Если пагинация не дала результатов, выходим
			if (!this.pickUpPagination.currentPage || !this.pickUpPagination.currentPage.length) {
				console.error("   - ОШИБКА: Пагинация не вернула текущую страницу с ПВЗ.");
			...
			var selectedStoreId = buyerStoreInput ? buyerStoreInput.value : 0;
			var storeFound = false;

			// Проверяем, есть ли уже выбранный ПВЗ среди доступных
			...
			// Если выбранного ПВЗ нет, автоматически выбираем первый
			if (!storeFound && stores.length > 0) {
				selectedStoreId = stores[0].ID;
				if (buyerStoreInput) buyerStoreInput.value = selectedStoreId;
			}

			// Создаем элементы списка
			for (var i = 0; i < this.pickUpPagination.currentPage.length; i++) {
				var currentStore = this.pickUpPagination.currentPage[i];
				var storeNode = this.createPickUpItem(currentStore, { selected: currentStore.ID == selectedStoreId });
				pickUpList.appendChild(storeNode);
			}

			// Добавляем заголовок и сам список в DOM
			...
			// 	})
			// );
			containerNode.appendChild(pickUpList);

			// Показываем пагинацию, если нужно
			if (this.pickUpPagination.show) {
				this.showPagination('pickUp', pickUpList);
			}

			// Инициализируем или обновляем карту
			if (this.params.SHOW_PICKUP_MAP === 'Y') {
				// Просто проверяем, готов ли наш объект карты. Без setTimeout.
				if (this.mapsReady && this.maps) {
					var selectedStoreObject = this.getSelectedPickUp();

					var mapContainer = BX('pickUpMap');
					if (mapContainer) {
						this.maps.initializePickUpMap(selectedStoreObject);
						this.maps.buildBalloons(stores);
					} else {
						// Эта ошибка теперь не должна появляться, но проверка полезна.
						console.error("   - ERROR: Map container #pickUpMap not found in DOM.");
					}
				}
				else {
					// Это штатная ситуация при первой загрузке страницы.
					// console.log("   - Map API is not ready yet. Map will be initialized later.");
				}
			}
		},
		...
		pickUpFinalAction: function () {
			...
			// Старая логика для "свернутого" блока больше не нужна, так как блок всегда видимый.
			...
			if (this.maps && typeof this.maps.pickUpFinalAction === 'function') {
				this.maps.pickUpFinalAction();
			}
			// >>> КОНЕЦ ИСПРАВЛЕНИЯ <<<
		},
		...
		createPickUpItem: function (currentStore, options) {
			...
			// --- НАЧАЛО ИЗМЕНЕНИЙ: Полностью новая верстка карточки ---
			var isSelected = !!options.selected;

			var html = '';
			if (currentStore.ADDRESS)
				html += '<div class="soa-pickup-address">' + BX.util.htmlspecialchars(currentStore.ADDRESS) + '</div>';

			var infoHtml = '';
			if (currentStore.PHONE)
				infoHtml += '<strong>' + BX.message('SOA_PICKUP_PHONE') + ':</strong> ' + BX.util.htmlspecialchars(currentStore.PHONE) + '<br>';
			if (currentStore.SCHEDULE)
				infoHtml += '<strong>' + BX.message('SOA_PICKUP_WORK') + ':</strong> ' + BX.util.htmlspecialchars(currentStore.SCHEDULE) + '<br>';

			if (infoHtml)
				html += '<div class="soa-pickup-info">' + infoHtml + '</div>';

			var storeNode = BX.create('DIV', {
				props: {
					className: 'bx-soa-pickup-list-item' + (isSelected ? ' bx-selected' : ''),
					id: 'store-' + currentStore.ID
				},
				children: [
					BX.create('DIV', {
						props: { className: 'bx-soa-pickup-l-item-detail' },
						html: html
					}),
					BX.create('DIV', {
						props: { className: 'bx-soa-pickup-l-item-btn' },
						children: [
							BX.create('SPAN', {
		...
		selectStore: function (event) {
			...
			if (storeItem && storeInput) {
				...
				// --- НАЧАЛО ИЗМЕНЕНИЙ: Обновляем текст на кнопках ---
				if (selectedPickUp) {
					BX.removeClass(selectedPickUp, 'bx-selected');
					var oldButton = selectedPickUp.querySelector('.btn');
					if (oldButton) oldButton.textContent = 'Выбрать';
				}

				BX.addClass(storeItem, 'bx-selected');
				var newButton = storeItem.querySelector('.btn');
				if (newButton) newButton.textContent = 'Выбран';
				// --- КОНЕЦ ИЗМЕНЕНИЙ ---
		...
		editPropsBlock: function (active) {
			...
			// Всегда рисуем только активный (редактируемый) вид блока
			this.editActivePropsBlock(true);
		},
		...
		editPropsItems: function (propsNode) {
			...
			while (group = groupIterator()) {
				...
				while (property = propsIterator()) {
					// --- НАЧАЛО ИЗМЕНЕНИЙ ---
					// Пропускаем свойства, которые мы перенесли в блок доставки
					var propCode = property.getSettings().CODE;
					if (this.propsToDelivery && this.propsToDelivery.indexOf(propCode) !== -1) {
						continue;
					}
					// --- КОНЕЦ ИЗМЕНЕНИЙ ---
		...
		getPropertyRowNode: function (property, propsItemsContainer, disabled) {
			...
			// --- НАЧАЛО ИЗМЕНЕНИЙ ---
			// Определяем классы для контейнера свойства
			var propertyClass = "form-group bx-soa-customer-field";
			if (propsItemsContainer.id === 'bx-soa-delivery-props-container') {
				propertyClass += ' col-sm-6'; // Делаем поля в 2 колонки
			}
			// --- КОНЕЦ ИЗМЕНЕНИЙ ---
			...
			if (disabled) {
				...
				BX.addClass(propsItemNode, propertyClass); // Применяем классы
		...
		renderDeliveryProperties: function (container) {
			if (!container || !this.propertyCollection) {
				return;
			}

			BX.cleanNode(container);

			var propsIterator = this.propertyCollection.getIterator();
			var property;

			// Перебираем все свойства заказа
			while (property = propsIterator()) {
				var settings = property.getSettings();
				var propCode = settings.CODE;
				var isZip = settings.IS_ZIP === 'Y'; // Проверяем, является ли свойство Индексом

				// --- ГЛАВНОЕ ИЗМЕНЕНИЕ ---
				// Отрисовываем свойство, если:
				// 1. Его код есть в массиве propsToDelivery (стандартное поведение)
				// ИЛИ
				// 2. Это свойство является Индексом (наше новое жесткое правило)
				if ((this.propsToDelivery && this.propsToDelivery.indexOf(propCode) !== -1) || isZip) {
					// Отрисовываем это свойство в переданном контейнере
		...
		isValidForm: function () {
			...
			var regionErrors = this.isValidRegionBlock(),
				...
				deliveryPropsErrors = this.isValidDeliveryPropertiesBlock(),
				// >>> НАЧАЛО ИСПРАВЛЕНИЯ: Объединяем ошибки местоположения и другие ошибки доставки в один массив <<<
				totalDeliveryErrors = regionErrors.concat(deliveryPropsErrors),
				// >>> КОНЕЦ ИСПРАВЛЕНИЯ <<<
				navigated = false, tooltips, i;

			// >>> НАЧАЛО ИСПРАВЛЕНИЯ: Проверяем объединенный массив ошибок и скроллим к правильному блоку доставки <<<
			if (totalDeliveryErrors.length && !navigated) {
				navigated = true;
			...
			// >>> НАЧАЛО ИСПРАВЛЕНИЯ: Отображаем все ошибки доставки в блоке доставки <<<
			if (totalDeliveryErrors.length) {
				this.showError(this.deliveryBlockNode, totalDeliveryErrors);
			...
			// >>> НАЧАЛО ИСПРАВЛЕНИЯ: Проверяем итоговую сумму ошибок <<<
			return !(propsErrors.length + totalDeliveryErrors.length);
			// >>> КОНЕЦ ИСПРАВЛЕНИЯ <<<
		},
		...
		isValidPropertiesBlock: function (excludeLocation) {
			...
			var props = this.orderBlockNode.querySelectorAll('#bx-soa-properties .bx-soa-customer-field[data-property-id-row]'), // <-- Уточнили селектор
				propsErrors = [],
				id, propContainer, arProperty, data, i;

			for (i = 0; i < props.length; i++) {
				// >>> НАЧАЛО ИСПРАВЛЕНИЯ №1.2: Пропускаем свойства, которые уже валидируются в доставке <<<
				// Эта проверка гарантирует, что мы не будем проверять одно и то же поле дважды.
				if (props[i].closest('#bx-soa-delivery-props-container')) {
					continue;
				}
				// >>> КОНЕЦ ИСПРАВЛЕНИЯ №1.2 <<<
		...
		showErrorTooltip: function (tooltipId, targetNode, text) {
			...
			var parentGroup = targetNode.parentNode;
			var errorMsgId = 'error-msg-' + tooltipId;
			var errorNode = BX(errorMsgId);

			text = this.uniqueText(text, '<br>');

			if (!errorNode) {
				errorNode = BX.create('DIV', {
					props: {
						id: errorMsgId,
						className: 'soa-field-error-message'
					}
				});
				parentGroup.appendChild(errorNode);
			}

			errorNode.innerHTML = text;

			// Используем setTimeout, чтобы CSS-анимация сработала после создания элемента
			setTimeout(function () {
				BX.addClass(errorNode, 'active');
			}, 10);
		},
		...
		closeErrorTooltip: function (tooltipId) {
			...
			if (errorNode) {
				BX.removeClass(errorNode, 'active');
			}
		},
		...
		// >>> НАЧАЛО ИЗМЕНЕНИЙ: Добавлены новые функции для корзины в сайдбаре <<<
		createSidebarBasketItem: function (itemData) {
			var itemContainer = BX.create('DIV', { props: { className: 'bx-soa-sidebar-basket-item' } });

			// Image
			var logotype = this.getImageSources(itemData, 'PREVIEW_PICTURE') || this.getImageSources(itemData, 'DETAIL_PICTURE');
			var imgSrc = (logotype && logotype.src_1x) || this.defaultBasketItemLogo;
			var imgNode = BX.create('IMG', { props: { src: imgSrc, alt: itemData.NAME } });
			var imgLink = BX.create('A', {
				props: { href: itemData.DETAIL_PAGE_URL || '#' },
				children: [imgNode]
			});
			var imageBlock = BX.create('DIV', {
				props: { className: 'bx-soa-sidebar-basket-item-image' },
				children: [imgLink]
			});

			// Details (Name, Quantity, Price)
			var nameNode = BX.create('DIV', {
				props: { className: 'bx-soa-sidebar-basket-item-name' },
				html: '<a href="' + (itemData.DETAIL_PAGE_URL || '#') + '">' + this.htmlspecialcharsEx(itemData.NAME) + '</a>'
			});
			var summaryNode = BX.create('DIV', {
				props: { className: 'bx-soa-sidebar-basket-item-summary' },
				html: itemData.QUANTITY + ' &times; ' + itemData.PRICE_FORMATED
			});
			var detailsBlock = BX.create('DIV', {
				props: { className: 'bx-soa-sidebar-basket-item-details' },
				children: [nameNode, summaryNode]
			});

			// Total
			var totalBlock = BX.create('DIV', {
				props: { className: 'bx-soa-sidebar-basket-item-total' },
				html: itemData.SUM
			});

			itemContainer.appendChild(imageBlock);
			itemContainer.appendChild(detailsBlock);
			itemContainer.appendChild(totalBlock);

			return itemContainer;
		},

		renderSidebarBasket: function () {
			var basketItemsContainer = BX('bx-soa-sidebar-basket-items');
			if (!basketItemsContainer || !this.result.GRID || !this.result.GRID.ROWS)
				return;

			BX.cleanNode(basketItemsContainer);

			var rows = this.result.GRID.ROWS;
			for (var i in rows) {
				if (rows.hasOwnProperty(i)) {
					var itemNode = this.createSidebarBasketItem(rows[i].data);
					basketItemsContainer.appendChild(itemNode);
				}
			}
		},
		// >>> КОНЕЦ ИЗМЕНЕНИЙ <<<
		...
		editTotalBlock: function () {
			// Эта функция рендерит товары в #bx-soa-sidebar-basket-items, она остается без изменений.
			this.renderSidebarBasket();

			// Находим наш новый контейнер для итоговых сумм
			var totalLinesNode = BX('bx-soa-total-lines');
			if (!totalLinesNode || !this.result.TOTAL) {
				return;
			}

			// Очищаем ТОЛЬКО контейнер с итогами, не трогая структуру и блок с товарами
			BX.cleanNode(totalLinesNode);
			...
			// ВАЖНО: Вместо this.totalInfoBlockNode.appendChild используем totalLinesNode.appendChild
			totalLinesNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_SUMMARY'), priceHtml, params));

			if (this.options.showOrderWeight) {
				totalLinesNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_WEIGHT_SUM'), total.ORDER_WEIGHT_FORMATED));
			}

			if (this.options.showTaxList) {
				for (i = 0; i < total.TAX_LIST.length; i++) {
					valFormatted = total.TAX_LIST[i].VALUE_MONEY_FORMATED || '';
					totalLinesNode.appendChild(
			...
			if (this.result.DELIVERY.length) {
				totalLinesNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, params));
			}
			...
			if (this.options.showDiscountPrice) {
				...
				totalLinesNode.appendChild(this.createTotalUnit(discText + ':', total.DISCOUNT_PRICE_FORMATED, { highlighted: true }));
			}

			if (this.options.showPayedFromInnerBudget) {
				totalLinesNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED));
				totalLinesNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_PAYED'), total.PAYED_FROM_ACCOUNT_FORMATED));
				totalLinesNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_LEFT_TO_PAY'), total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED, { total: true }));
			} else {
				totalLinesNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED, { total: true }));
			}

			if (parseFloat(total.PAY_SYSTEM_PRICE) >= 0 && this.result.DELIVERY.length) {
				totalLinesNode.appendChild(this.createTotalUnit(BX.message('SOA_PAYSYSTEM_PRICE'), '~' + total.PAY_SYSTEM_PRICE_FORMATTED));
			}

			if (this.params.SHOW_COUPONS_BASKET === 'Y') {
				totalLinesNode.appendChild(this.renderCouponBlock());
			}
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								...
								// УБЕДИТЕСЬ, ЧТО ЭТОТ ОБРАБОТЧИК ЕСТЬ
								events: {
			...
			// >>> НАЧАЛО ИСПРАВЛЕНИЯ: Динамическое создание блока согласия <<<
			var consentContainer = BX('bx-soa-user-consent-container');
			if (consentContainer) {
				BX.cleanNode(consentContainer); // Очищаем контейнер перед новой вставкой

				if (this.params.USER_CONSENT === 'Y') {
					var formCheck = BX.create('DIV', { props: { className: 'form-check' } });
					...
					var checkbox = BX.create('INPUT', {
						props: {
							...
							type: 'checkbox',
							value: 'Y',
							id: 'soa-user-consent', // Уникальный ID будет создан здесь
							name: 'USER_CONSENT',
							checked: true
						}
					});
					...
					var label = BX.create('LABEL', {
						...
						attrs: { for: 'soa-user-consent' },
						children: [
							BX.create('SPAN', {
								props: { className: 'form-checbox__text' },
								html: 'Продолжая, вы соглашаетесь с <a href="/privacy-policy/" target="_blank">политикой конфиденциальности</a>'
							})
						]
					});

					var errorDiv = BX.create('DIV', {
						props: {
							id: 'soa-user-consent-error', // Уникальный ID для ошибки
							className: 'soa-field-error-message'
						}
					});
					...
					consentContainer.appendChild(formCheck);
				}
			}
			// >>> КОНЕЦ ИСПРАВЛЕНИЯ <<<

			this.editMobileTotalBlock(); // Эта строка должна быть ПОСЛЕ вставленного кода
			this.totalBlockFixFont();
		},

		// >>> НАЧАЛО ДОБАВЛЕНИЯ НОВОЙ ФУНКЦИИ <<<
		renderCouponBlock: function () {
			var couponsList = this.getCouponsList(true), // true - для активного отображения (с кнопкой удаления)
				couponsLabel = this.getCouponsLabel(true), // true - для активного отображения (с текстом "Применить купон:")
				couponsBlock = BX.create('DIV', {
					props: { className: 'bx-soa-coupon-block' },
					children: [
						BX.create('DIV', {
							...
							children: [
								BX.create('INPUT', {
									props: {
										className: 'form-control bx-ios-fix',
										type: 'text',
										placeholder: BX.message('COUPON_PLACEHOLDER') // Добавляем плейсхолдер для поля
									},
									events: {
										change: BX.delegate(function (event) {
											var newCoupon = BX.getEventTarget(event);
											if (newCoupon && newCoupon.value) {
												this.sendRequest('enterCoupon', newCoupon.value);
												newCoupon.value = ''; // Очищаем поле после отправки
											}
										}, this)
									}
								})
		...
		totalBlockScrollCheck: function () {
			// Если сайдбара нет или это мобильное устройство, ничего не делаем
			if (!this.totalInfoBlockNode || !this.totalGhostBlockNode || window.innerWidth <= 991) {
			...
			// Главный контейнер, который содержит и контент, и сайдбар
			var parentContainer = BX('bx-soa-order');
			if (!parentContainer) return;

			var parentPos = BX.pos(parentContainer);
			var ghostPos = BX.pos(this.totalGhostBlockNode);

			var sidebarHeight = this.totalInfoBlockNode.offsetHeight;
			var sidebarWidth = this.totalGhostBlockNode.offsetWidth;

			// Нижняя граница, где сайдбар должен "отлипнуть" от верха и "прилипнуть" к низу
			// Это нижняя точка родительского контейнера минус высота самого сайдбара
			var bottomBoundary = parentPos.bottom - sidebarHeight - 20; // 20px - небольшой отступ

			// 1. Если мы прокрутили так далеко, что сайдбар упирается в "дно"
			if (scrollTop >= bottomBoundary) {
				BX.removeClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed');
				BX.addClass(this.totalInfoBlockNode, 'bx-soa-cart-total-bottom');
				this.totalInfoBlockNode.style.width = sidebarWidth + 'px';
				this.totalGhostBlockNode.style.height = sidebarHeight + 'px';
			}
			// 2. Если мы прокрутили ниже начальной позиции сайдбара, но еще не достигли "дна"
			else if (scrollTop > ghostPos.top) {
				BX.removeClass(this.totalInfoBlockNode, 'bx-soa-cart-total-bottom');
				BX.addClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed');
				this.totalInfoBlockNode.style.width = sidebarWidth + 'px';
				this.totalGhostBlockNode.style.height = sidebarHeight + 'px'; // Занимаем место, чтобы контент не прыгал
			}
			// 3. Если мы находимся вверху страницы
			else {
				BX.removeClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed');
				BX.removeClass(this.totalInfoBlockNode, 'bx-soa-cart-total-bottom');
				this.totalInfoBlockNode.style.width = '';
				this.totalGhostBlockNode.style.height = ''; // Возвращаем все в исходное состояние
			}
		},
