...
(function () {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function (parameters) {
			...
			this.regionBlockNode = BX(parameters.regionBlockId) || { id: 'bx-soa-region', style: {} };
			this.regionHiddenBlockNode = BX(parameters.regionBlockId + '-hidden') || { id: 'bx-soa-region-hidden', style: {} };
			...
			this.pickUpBlockNode = BX(parameters.pickUpBlockId) || { id: 'bx-soa-pickup', style: {} };
			this.pickUpHiddenBlockNode = BX(parameters.pickUpBlockId + '-hidden') || { id: 'bx-soa-pickup-hidden', style: {} };
		...
		refreshOrder: function (result) {
			if (result.error) {
				...
				if (!this.mapsReady) {
					...
					// При AJAX-обновлении нам нужно переинициализировать только ПВЗ
		...
		locationsCompletion: function () {
			...
			/* 
			 *  ВОТ ЭТОТ БЛОК МЫ УДАЛИЛИ. ОН ПЕРЕЗАПИСЫВАЛ РЕДАКТИРУЕМЫЕ ПОЛЯ НА ТЕКСТ.
			 *
		...
		clickOrderSaveAction: function (event) {
			if (this.isValidForm()) // <--- Здесь происходит валидация
			{
				this.allowOrderSave();
		...
		show: function (node) {
			...
			// --- НАЧАЛО ИСПРАВЛЕНИЯ ---
			// Пропускаем несуществующие блоки
			if (node.id === 'bx-soa-region' || node.id === 'bx-soa-pickup') {
				return;
			}
			// --- КОНЕЦ ИСПРАВЛЕНИЯ ---
		...
		showActualBlock: function () {
			...
			while (allSections[i]) {
				...
				if (section.id === this.regionBlockNode.id || section.id === this.pickUpBlockNode.id) {
					i++;
					continue;
				}

				if (section.id === this.propsBlockNode.id)
			...
			for (i = 0; i < sections.length; i++) {
				...
				if (section) {
					if (section.id === 'bx-soa-region' || section.id === 'bx-soa-pickup') {
						if (BX(section.id)) { // Дополнительная проверка
		...
		editSection: function (section) {
			...
			switch (section.id) {
				...
				case this.basketBlockNode.id:
					...
					break;
				case 'bx-soa-region': // Используем строку, а не this.regionBlockNode.id
					// Do nothing, handled by editDeliveryBlock
					break;
				case this.paySystemBlockNode.id:
				...
				case this.deliveryBlockNode.id:
					...
					break;
				case 'bx-soa-pickup': // Используем строку
					// Do nothing, handled by checkPickUpShow
					break;
				case this.propsBlockNode.id:
		...
		// Добавляем два новых аргумента: container (куда рисовать) и node (откуда брать данные)
		editActiveRegionBlock: function (activeNodeMode, container) {
			...
			if (!container) {
				container = node.querySelector('.bx-soa-section-content');
				if (!container) {
					container = this.getNewContainer();
					if (node && node.appendChild) node.appendChild(container);
				} else {
		...
		editActiveDeliveryBlock: function (activeNodeMode) {
			...
			if (this.initialized.delivery) {
				...
				this.editActiveRegionBlock(true, regionContainer);
		...
		// ЗАМЕНА 3: createDeliveryItem
		createDeliveryItem: function (item) {
			var checked = item.CHECKED == 'Y',
				deliveryId = parseInt(item.ID),
				deliveryCached = this.deliveryCachedInfo[deliveryId],
				logotype, logoNode,
				name, itemNode, itemInnerContainer;

			name = this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME;

			itemInnerContainer = BX.create('DIV', {
				props: { className: 'bx-soa-pp-company-inner' }
			});

			itemNode = BX.create('DIV', {
				props: { className: 'bx-soa-pp-company col-6' },
				children: [itemInnerContainer],
			...
			var grafContainer = BX.create('DIV', {
				props: {
				...
				children: [
					...
					BX.create('LABEL', {
						...
						props: { className: 'bx-soa-pp-company-label' },
						html: '<span>' + BX.util.htmlspecialchars(name) + '</span>'
					})
				]
			});
			...
			logoNode.style.backgroundImage = 'url("' + (logotype && logotype.src_1x || this.defaultDeliveryLogo) + '")';
			...
			var priceNode = BX.create('DIV', {
				props: { className: 'bx-soa-pp-delivery-cost' },
				html: priceHtml || (this.params.MESS_PRICE_FREE || 'бесплатно')
			});
			...
			var pickupContainer = BX.create('DIV', {
				props: {
					id: 'pickup-for-delivery-' + deliveryId,
					className: 'pickup-in-delivery-container'
				},
				style: {
					display: 'none'
				}
			...
			itemInnerContainer.appendChild(grafContainer);
			itemInnerContainer.appendChild(logoNode);
			itemInnerContainer.appendChild(priceNode);
			itemInnerContainer.appendChild(descriptionNode);
			itemInnerContainer.appendChild(pickupContainer);
		...
		selectDelivery: function (event) {
			...
			var actionInput = actionSection.querySelector('input[type=radio]');

			if (selectedSection) {
				BX.removeClass(selectedSection, 'bx-selected');
				var selectedInput = selectedSection.querySelector('input[type=radio]');
				if (selectedInput) {
		...
		getSelectedDelivery: function () {
			// !!! ИСПРАВЛЕНИЕ: ищем input[type=radio] !!!
			var deliveryRadio = this.deliveryBlockNode.querySelector('input[type=radio][name=DELIVERY_ID]:checked'),
				currentDelivery = false,
				deliveryId, i;

			if (!deliveryRadio)
				deliveryRadio = this.deliveryHiddenBlockNode.querySelector('input[type=radio][name=DELIVERY_ID]:checked');
		...
		deactivatePickUp: function () {
			var allPickupContainers = this.orderBlockNode.querySelectorAll('.pickup-in-delivery-container');
			for (var i = 0; i < allPickupContainers.length; i++) {
				allPickupContainers[i].style.display = 'none';
				BX.cleanNode(allPickupContainers[i]);
			}

			if (this.pickUpBlockNode && BX.hasClass(this.pickUpBlockNode, 'bx-active')) {
		...
		editActivePickUpBlock: function (activeNodeMode, container) {
			...
			var pickUpContentCol = BX.create('DIV', { props: { className: 'col' } });

			if (this.params.SHOW_PICKUP_MAP == 'Y') {
				// Просто создаем div для карты. ID должен быть уникальным!
				var mapId = 'pickUpMap_' + BX.util.getRandomString(5);
				pickUpContentCol.appendChild(BX.create('DIV', {
					props: { id: mapId },
					style: { width: '100%', marginBottom: '10px', height: '400px' } // Задаем высоту
				}));
			}

			pickUpContentCol.appendChild(
				BX.create('DIV', { props: { id: 'pickUpLoader', className: 'text-center' } })
			);
			...
			if (this.mapsReady && this.maps) {
				...
				if (currentDelivery && currentDelivery.STORE && currentDelivery.STORE.length) {
					...
					if (activeStores && activeStores.length) {
						var selectedStore = this.getSelectedPickUp();

						// Ждем микропаузу, чтобы div карты точно был в DOM
						setTimeout(BX.delegate(function () {
							console.log("Attempting to initialize map on #" + mapId);
							this.maps.initializePickUpMap(selectedStore, mapId); // Передаем ID контейнера
							this.maps.buildBalloons(activeStores);
							this.resizeMapContainers(); // Подгоняем размер
						}, this), 100);
					}
				}
			}
		...
		checkPickUpShow: function () {
			console.log('--- checkPickUpShow triggered ---'); // 1. Проверяем, что функция вызвана
			this.deactivatePickUp();

			var currentDelivery = this.getSelectedDelivery();
			if (!currentDelivery) {
				console.log('No delivery selected.'); // 2. Проверяем, нашлась ли выбранная доставка
				return;
			}
			console.log('Selected delivery:', currentDelivery); // 3. Смотрим, что за доставка выбрана

			var stores = (currentDelivery.STORE && currentDelivery.STORE.length > 0)
				? this.getPickUpInfoArray(currentDelivery.STORE)
				: [];

			console.log('Found stores for this delivery:', stores); // 4. Проверяем, нашлись ли для нее склады (ПВЗ)

			if (stores.length > 0) {
				console.log('Stores found! Looking for container: #pickup-for-delivery-' + currentDelivery.ID); // 5. Ищем контейнер
				var pickupContainer = BX('pickup-for-delivery-' + currentDelivery.ID);
				if (pickupContainer) {
					console.log('Container found! Showing it and rendering pickup block.'); // 6. Контейнер найден, показываем!
					pickupContainer.style.display = '';
					this.editPickUpBlock(true, pickupContainer);
				} else {
					console.error('ERROR: Container #pickup-for-delivery-' + currentDelivery.ID + ' NOT FOUND!'); // 7. Ошибка, если контейнер не нашелся
				}
			} else {
				console.log('No stores for this delivery.'); // 8. Для этой доставки нет ПВЗ
			}
		},
