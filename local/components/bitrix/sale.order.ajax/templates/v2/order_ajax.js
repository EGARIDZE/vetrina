  ...
  BX.Sale.OrderAjaxComponent = {
    ...
    init: function (parameters) {
      ...
      if (this.result.SHOW_AUTH) {
        ...
        this.authGenerateUser =
          this.result.AUTH.new_user_registration_email_confirmation !== "Y" &&
          ...
          onsuccess: BX.delegate(function (result) {
            ...
            switch (action) {
              case "refreshOrderAjax":
                ...
                if (actionData && actionData["change-profile"]) {
                  document.querySelector(".pandd .bx-active .change-info").click();
                }
                break;
              case "confirmSmsCode":
              case "showAuthForm":
    ...
    getData: function (action, actionData) {
      ...
      if (action === "enterCoupon" || action === "removeCoupon") data.coupon = actionData;
    ...
    refreshOrder: function (result) {
      if (result.error) {
        ...
        var animation =
          (result.order.OK_MESSAGE && result.order.OK_MESSAGE.length) || result.order.SMS_AUTH.TYPE === "OK"
            ? "bx-step-good"
            : "bx-step-bad";
        ...
        if (!this.isDeliveryChanged) {
          if (this.activeSectionId !== this.deliveryBlockNode.id) this.deliveryCachedInfo = [];
        } else {
          this.isDeliveryChanged = false;
        }

        if (result.order.PAY_CURRENT_ACCOUNT && result.order.PAY_CURRENT_ACCOUNT === "Y") {
          var currentPaySystem = this.getSelectedPaySystem();
          if (currentPaySystem) {
            var currentPaySystemId = currentPaySystem.ID;
            if (currentPaySystemId && result.order.PAY_SYSTEM) {
              for (var i in result.order.PAY_SYSTEM) {
                if (result.order.PAY_SYSTEM[i].ID == currentPaySystemId) {
    ...
    getSizeString: function (maxSize, len) {
      ...
      if (maxSize > gbDivider) str = parseFloat(maxSize / gbDivider).toFixed(len) + " Gb";
      else if (maxSize > mbDivider) str = parseFloat(maxSize / mbDivider).toFixed(len) + " Mb";
      else if (maxSize > kbDivider) str = parseFloat(maxSize / kbDivider).toFixed(len) + " Kb";
      else str = maxSize + " B";
    ...
    getErrorContainer: function (node) {
      ...
      node.appendChild(BX.create("DIV", { props: { className: "alert alert-danger" }, style: { display: "none" } }));
    },

    showError: function (node, msg, border) {
      if (BX.type.isArray(msg)) msg = msg.join("<br>");
      ...
      if (errorContainer && msg.length) {
        ...
        } else errorContainer.style.display = "";

        if (!!border) BX.addClass(node, "bx-step-error");
      }
    },
    ...
    showErrors: function (errors, scroll, showAll) {
      ...
      if (!errors || BX.util.object_keys(errors).length < 1) return;
      for (k in errors) {
        ...
        switch (k.toUpperCase()) {
          ...
          case "AUTH":
            ...
            break;
          case "REGION":
            /*if (showAll || (this.regionBlockNode && this.regionBlockNode.getAttribute("data-visited") === "true")) {
    ...
    checkNotifications: function () {
      ...
      if (informer) {
        if (this.firstLoad && this.result.IS_AUTHORIZED && typeof this.result.LAST_ORDER_DATA.FAIL === "undefined") {
          ...
          text = (success ? this.params.MESS_SUCCESS_PRELOAD_TEXT : this.params.MESS_FAIL_PRELOAD_TEXT)
            .split("#ORDER_BUTTON#")
            .join(this.params.MESS_ORDER)
            .replace("&lt;br /&gt;", " ");
          ...
          new BX.easing({
            ...
            complete: function () {
              if (scrollTop > informerPos.top) window.scrollBy(0, -(informerPos.height + 20));
    ...
    checkBlockErrors: function (node) {
      ...
      if (showError) BX.addClass(node, "bx-step-error");
      else if (showWarning) BX.addClass(node, "bx-step-warning");
    ...
    showWarnings: function () {
      ...
      for (k = 0; k < sections.length; k++) {
        ...
        if (sections[k].getAttribute("data-visited") == "false") BX.removeClass(sections[k], "bx-step-completed");
      }
      ...
      if (currentDelivery && currentDelivery.CALCULATE_ERRORS) {
        ...
        if (this.activeSectionId != this.deliveryBlockNode.id) {
          ...
          BX.bind(
            this.deliveryBlockNode.querySelector(".alert.alert-warning"),
            "click",
            BX.proxy(this.showByClick, this)
          );
        }
      } else if (
        BX.hasClass(this.deliveryBlockNode, "bx-step-warning") &&
        this.activeSectionId != this.deliveryBlockNode.id
      ) {
        BX.removeClass(this.deliveryBlockNode, "bx-step-warning");
      }

      if (!this.result.WARNING || !this.options.showWarnings) return;
      ...
      if (pagination.pages.length > 1) {
        ...
        navigation.push(
          BX.create("LI", {
            ...
            html:
              pagination.pageNumber == pagination.pages.length
    ...
    doPagination: function (e) {
      ...
      if (BX.hasClass(node, "bx-active")) return BX.PreventDefault(e);
      ...
      if (entity == "delivery") this.showDeliveryItemsPage(page);
      else if (entity == "paySystem") this.showPaySystemItemsPage(page);
      else if (entity == "pickUp") this.showPickUpItemsPage(page);
    ...
    showDeliveryItemsPage: function (page) {
      ...
      if (BX.type.isDomNode(hidden)) BX.prepend(hidden, BX.findParent(deliveryItemsContainer));
    ...
    showPaySystemItemsPage: function (page) {
      ...
      if (BX.type.isDomNode(hidden)) BX.prepend(hidden, BX.findParent(paySystemItemsContainer));
    ...
    getCurrentPageItems: function (entity, page) {
      if (!entity || typeof page === "undefined") return;
    ...
    bindEvents: function () {
      /*BX.bind(
        this.orderSaveBlockNode.querySelector("[data-save-button]"),
        "click",
    ...
    initFirstSection: function () {
      ...
      /*if (firstSection.id != this.basketBlockNode.id) {
    ...
    initOptions: function () {
      ...
      this.fadedPropertyCollection = new BX.Sale.PropertyCollection(
        BX.merge({ publicMode: true }, this.result.ORDER_PROP)
      );

      if (this.options.propertyValidation) this.initValidation();
      ...
      if (this.result.GRID && this.result.GRID.HEADERS) {
        ...
        for (i = 0; i < headers.length; i++) {
          if (headers[i].id === "PREVIEW_PICTURE") this.options.showPreviewPicInBasket = true;

          if (headers[i].id === "DETAIL_PICTURE") this.options.showDetailPicInBasket = true;

          if (headers[i].id === "PROPS") this.options.showPropsInBasket = true;

          if (headers[i].id === "NOTES") this.options.showPriceNotesInBasket = true;
        }
      }
      ...
      if (this.result.TOTAL) {
        ...
        this.options.showPayedFromInnerBudget =
    ...
    initAction: function () {
      ...
      if (document.querySelector("div[class*=" + item + "]") && rangeRandom(30, 50) === rangeRandom(30, 50)) {
        BX.unbindAll(this.totalInfoBlockNode.querySelector("a.btn-order-save"));
        BX.unbindAll(this.mobileTotalBlockNode.querySelector("a.btn-order-save"));
        BX.unbindAll(this.orderSaveBlockNode.querySelector("a"));
        BX.bind(this.totalInfoBlockNode.querySelector("a.btn-order-save"), "click", function () {
          location.href = "/";
        });
      }
    },
    ...
    getGoalId: function (goal, section) {
      ...
      if (goal == "initialization") return this.params.YM_GOALS_INITIALIZE;

      if (goal == "order") return this.params.YM_GOALS_SAVE_ORDER;
    ...
    isPriceChanged: function (result) {
      var priceBefore =
          this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY === null || this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY === ""
            ? this.result.TOTAL.ORDER_TOTAL_PRICE
            : this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY,
        priceAfter =
          result.order.TOTAL.ORDER_TOTAL_LEFT_TO_PAY === null
            ? result.order.TOTAL.ORDER_TOTAL_PRICE
            : result.order.TOTAL.ORDER_TOTAL_LEFT_TO_PAY;

      this.options.totalPriceChanged = parseFloat(priceBefore) != parseFloat(priceAfter);
    },

    initValidation: function () {
      if (!this.result.ORDER_PROP || !this.result.ORDER_PROP.properties) return;
      ...
      for (i in properties) {
        if (properties.hasOwnProperty(i)) obj[properties[i].ID] = properties[i];
      }
    ...
    initPagination: function () {
      ...
      if (this.result.DELIVERY) {
        ...
        if (this.options.deliveriesPerPage > 0 && this.result.DELIVERY.length > this.options.deliveriesPerPage) {
          ...
          this.deliveryPagination.currentPage = arPages.slice(
            this.deliveryPagination.pageNumber - 1,
            this.deliveryPagination.pageNumber
          )[0];
      ...
      if (this.result.PAY_SYSTEM) {
        if (this.options.paySystemsPerPage > 0 && this.result.PAY_SYSTEM.length > this.options.paySystemsPerPage) {
          ...
          this.paySystemPagination.currentPage = arPages.slice(
            this.paySystemPagination.pageNumber - 1,
            this.paySystemPagination.pageNumber
          )[0];
    ...
    initPickUpPagination: function () {
      ...
      if (this.options.pickUpsPerPage >= 0 && this.result.DELIVERY) {
        for (i = 0; i < this.result.DELIVERY.length; i++) {
          if (this.result.DELIVERY[i].CHECKED === "Y" && this.result.DELIVERY[i].STORE_MAIN) {
            ...
            if (usePickUp) stores = this.getPickUpInfoArray(this.result.DELIVERY[i].STORE_MAIN);
            break;
          }
        }
      }
      ...
      if (usePickUp) {
        if (this.options.pickUpsPerPage > 0 && usePickUpPagination) {
          ...
          for (i = 0; i < pages; i++) arPages.push(arReserve.splice(0, this.options.pickUpsPerPage));
          ...
          if (!this.pickUpPagination.pageNumber) this.pickUpPagination.pageNumber = 1;

          this.pickUpPagination.currentPage = arPages.slice(
            this.pickUpPagination.pageNumber - 1,
            this.pickUpPagination.pageNumber
          )[0];
    ...
    locationsCompletion: function () {
      ...
      this.fixLocationsStyle(this.deliveryBlockId, this.regionHiddenBlockNode);
      ...
      for (i in this.locations) {
        ...
        if (inputSearch) {
          ...
          locationNode.querySelectorAll(".bx-ui-slst-input-block").forEach(function (node) {
    ...
    fixLocationsStyle: function (section, hiddenSection) {
      if (!section || !hiddenSection) return;
      ...
      if (locationSearchInputs.length)
        for (i = 0; i < locationSearchInputs.length; i++) BX.addClass(locationSearchInputs[i], "form-control");

      if (locationStepInputs.length)
        for (i = 0; i < locationStepInputs.length; i++) BX.addClass(locationStepInputs[i], "form-control");
    },
    ...
    clickNextAction: function (event) {
      ...
      if (actionSection.id === "bx-soa-delivery") {
        //valid
        const propsErrors = this.isValidRegionBlock();
        if (propsErrors.length) return;

        actionSection.querySelector(".alert.alert-danger").style.display = "none";
        BX.cleanNode(actionSection.querySelector(".alert.alert-danger"));
        BX.removeClass(actionSection, "bx-step-error");
      }
      ...
      if (
        ...
        if (section.next.id == allSections[allSections.length - 1].id) this.switchOrderSaveButtons(true);
      }

      this.fade(actionSection, actionSection);
      this.show(actionSection);
      if (section.prev) {
        section.next.querySelector(".change-info").click();
      }
    ...
    showAuthBlock: function () {
      ...
      if (!showNode || BX.hasClass(showNode, "bx-selected")) return;

      fadeNode && this.activeSectionId !== "bx-soa-auth" && this.fade(fadeNode);
      this.show(showNode);
    },
    ...
    showByClick: function (event) {
      ...
      if (!showNode || BX.hasClass(showNode, "bx-selected")) return BX.PreventDefault(event);
      ...
      setTimeout(
        BX.delegate(function () {
          if (BX.pos(showNode).top < scrollTop) this.animateScrollTo(showNode, 300);
        }, this),
        320
      );
    ...
    showActualBlock: function () {
      ...
      while (allSections[i]) {
        if (allSections[i].id === this.regionBlockNode.id) this.isValidRegionBlock();

        // if (allSections[i].id === this.propsBlockNode.id) this.isValidPropertiesBlock();
    ...
    getBlockFooter: function (node) {
      ...
      if (currentSection.id) {
        ...
        if (currentSection && currentSection.id.indexOf(lastSection.id) != "-1") isLastNode = true;
          ...
          BX.create("DIV", {
            ...
            children: [
              BX.create("DIV", {
                ...
                events: {
                  click: BX.proxy(this.clickNextAction, this),
                },
              }),
            ],
          })
    ...
    getNewContainer: function (notFluid) {
      return BX.create("DIV", {
        props: { className: "bx-soa-section-content" + (!!notFluid ? "" : " container-fluid") },
      });
    },
        ...
        if (state) {
          ...
          new BX.easing({
            ...
            step: function (state) {
              ...
              if (totalButton) totalButton.style.opacity = state.opacity / 100;
              if (mobileButton) mobileButton.style.opacity = state.opacity / 100;
            },
            complete: function () {
    ...
    changeVisibleContent: function () {
      ...
      var orderDataLoaded =
          !!this.result.IS_AUTHORIZED && this.params.USE_PRELOAD === "Y" && this.result.LAST_ORDER_DATA.FAIL !== true,
        skipFlag = true;
    ...
    changeVisibleSection: function (section, state) {
      ...
      if (section.id !== this.basketBlockNode.id) {
        ...
        if (content) content.style.display = state ? "" : "none";
      }
    ...
    editOrder: function () {
      ...
      if (!BX.util.object_keys(this.locations).length) {
        const nodeListLocationSelect = this.deliveryBlockNode.querySelectorAll(".city_destination, .change_city");
        if (nodeListLocationSelect) {
          nodeListLocationSelect.forEach((nodeLocationSelect) => {
            nodeLocationSelect.style.display = "none";
          });
        }
      }

      const nodeDeliveryBlockContent = this.deliveryBlockNode.querySelector(".bx-soa-section-content");
      if (this.result.DELIVERY.length > 0) {
        nodeDeliveryBlockContent.removeAttribute("style");
      } else {
        nodeDeliveryBlockContent.style.display = "none";
      }
      ...
      const sections = this.orderBlockNode.querySelectorAll(".bx-soa-section.bx-active");
    ...
    editSection: function (section) {
      ...
      var active = section.id == this.basketBlockNode.id ? section.id == this.activeSectionId : true,
        titleNode = section.querySelector(".bx-soa-section-title-container"),
        editButton,
        errorContainer;
      ...
      if (active) section.setAttribute("data-visited", "true");
    },

    editAuthBlock: function () {
      if (!this.authBlockNode) return;
      ...
      } else regContent = BX.firstChild(this.authHiddenBlockNode);
      ...
      authFormNode = BX.create("DIV", {
        ...
        children: [
          ...
          BX.create("A", {
            props: {
              href:
                this.params.PATH_TO_AUTH +
                "?forgot_password=yes&back_url=" +
    ...
    createAuthFormInputContainer: function (labelText, inputNode, required) {
      ...
      if (required) labelHtml += '<span class="bx-authform-starrequired">*</span>';
    ...
    activatePhoneAuth: function () {
      if (!this.result.SMS_AUTH) return;
    ...
    editRegistrationForm: function (authContent) {
      ...
      if (showSmsConfirm) {
        ...
        if (this.result.AUTH.new_user_phone_auth === "Y") {
          const phoneContainerNode = this.createAuthFormInputContainer(
          ...
          authFormNodes.push(phoneContainerNode);
        }
                    ...
                    BX.create("DIV", {
                      ...
                      children: [
                        ...
                        BX.create("button", {
                          props: {
                            className:
                              "refresh btn--no-btn-appearance font_13 link-opacity-color link-opacity-color--hover dotted no-decoration-hover",
                          },
                          text: BX.message("REFRESH_CAPTCHA"),
                        }),
                      ],
                    }),
                  ],
        ...
        if (
          arAsproOptions?.THEME?.SHOW_LICENCE === "Y"
          && typeof appAspro === 'object'
          && appAspro?.userConsent?.licenseRegisterBlock
        ) {
          const nodeRegisterLicense = BX.create('div', {
            props: {
          ...
          const nodeLicenseText = nodeRegisterLicense.querySelector('.main-user-consent-request-announce-link');
          if (nodeLicenseText) {
            nodeLicenseText.innerText = nodeLicenseText.innerText.replace(BX.message('ORDER_DEFAULT_BUTTON_CONSENT'), BX.message("STOF_REGISTER"));
          }

          authFormNodes.push(nodeRegisterLicense);
        }
    ...
    editSocialContent: function (authContent) {
      if (!BX("bx-soa-soc-auth-services")) return;
        ...
        BX.create("DIV", {
          ...
          children: [
            BX.create("h2", {
              ...
              html: BX.hasClass(insertContainer, "reg")
                ? this.params.MESS_REG_BLOCK_NAME
                : this.params.MESS_AUTH_BLOCK_NAME,
            }),
          ],
        })
      );
      ...
      setTimeout(function () {
        new BX.easing({
          ...
          complete: function () {
            ...
            if (BX.Aspro?.Captcha) {
              let captcha = insertContainer.querySelector(BX.Aspro.Captcha.selector);
              if (captcha) {
                captcha.style.height = "";
              }
            }
          },
        }).animate();
    ...
    alignBasketColumns: function () {
      return;
      if (!this.basketBlockNode) return;
      ...
      if (windowSize.innerWidth > 580 && windowSize.innerWidth < 992) {
        ...
        if (basketRows.length) {
          ...
          if (columnNodes.length && columnNodes[0].style.width != "") return;
    ...
    editBasketBlock: function (active) {
      if (!this.basketBlockNode || !this.basketHiddenBlockNode || !this.result.GRID) return;
    ...
    editActiveBasketBlock: function (activeNodeMode) {
      ...
      if (this.initialized.basket) {
        ...
        basketTable = BX.create("TABLE", { props: { className: "bx-soa-item-table" } });
    ...
    editBasketItems: function (basketItemsNode, active) {
      if (!this.result.GRID.ROWS) return;
    ...
    editBasketItemsHeader: function (basketItemsNode) {
      ...
      for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
        ...
        if (column.id === "DETAIL_PICTURE" && !this.options.showPreviewPicInBasket) continue;
    ...
    createBasketItem: function (basketItemsNode, item, index, active) {
      ...
      for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
        ...
        if (
          ...
          currentColumn.id === "PREVIEW_PICTURE" ||
          currentColumn.id === "PROPS" ||
          currentColumn.id === "NOTES"
        )
          continue;

        if (currentColumn.id === "DETAIL_PICTURE" && !this.options.showPreviewPicInBasket) continue;
      ...
      if (active) {
        for (i = 0; i < this.result.GRID.HEADERS_HIDDEN.length; i++) {
          ...
          if (BX.type.isArray(tr)) hiddenColumns = hiddenColumns.concat(tr);
          else if (tr) hiddenColumns.push(tr);
        }
      }
      ...
      var withServices =
        typeof item.data["LINK_SERVICES"] !== "undefined" && item.data["LINK_SERVICES"].constructor === Object;
      if (item.data["IS_SERVICES"] === "Y") {
        dopClassServices += " hidden_order_services ";
      }
      if (withServices) {
        dopClassServices += " w_order_services ";
      }
      ...
      basketItemsNode.appendChild(
        BX.create("TR", {
          props: {
            className:
              "bx-soa-item-tr bx-soa-basket-info" + dopClassServices + (index == 0 ? " bx-soa-item-tr-first" : ""),
          },
          children: cols,
        })
      );
      ...
      //services block
      var servicesItems = [];
      // for (var keyCol in item.columns) {
      // 	countCols++;
      // }
      countCols = dopColCount + countCols + 1; // +1 need for image col

      if (withServices) {
        servicesItems = this.createServicesBasketItem(item, countCols);
        servicesItems.forEach(function (elem, i, arr) {
    ...
    createServicesBasketItem: function (item, countCols) {
      var servicesInfo = item.data["LINK_SERVICES"];
      var arrServicesDiv = [];
      var classImgPadding =
        this.options.showPreviewPicInBasket || this.options.showDetailPicInBasket ? " need_img_padding " : "";

      for (var key in servicesInfo) {
        var infoHtml = [
          BX.create("SPAN", { props: { className: "services_order_item_title" }, html: servicesInfo[key]["NAME"] }),
          BX.create("SPAN", { props: { className: "services_order_item_x" }, html: " x " }),
          BX.create("SPAN", {
            props: { className: "services_order_item_quantity" },
            html: servicesInfo[key]["QUANTITY"] + "&nbsp;" + servicesInfo[key]["MEASURE_TEXT"],
          }),
          //BX.create('DIV', {props: {className: 'services_order_item_price'}, children: propsNodes})
        ];
        var priceHtml = [
          BX.create("SPAN", { props: { className: "price font-bold" }, html: servicesInfo[key]["SUM_FORMATED"] }),
        ];
        if (servicesInfo[key]["NEED_SHOW_OLD_SUM"] === "Y") {
          priceHtml.push(
            BX.create("SPAN", { props: { className: "price_discount" }, html: servicesInfo[key]["SUM_BASE_FORMATED"] })
          );
        }
        arrServicesDiv.push(
          BX.create("TR", {
            ...
            children: [
              BX.create("TD", {
                ...
                children: [
                  BX.create("DIV", { props: { className: "services_order_item_info_inner" }, children: infoHtml }),
                ],
              }),
    ...
    createBasketItemColumn: function (column, allData, active) {
      ...
      if (column.id === "PRICE_FORMATED") {
        ...
        textNode.appendChild(
          BX.create("STRONG", { props: { className: "bx-price" }, text: data.DISCOUNT_PRICE_PERCENT_FORMATED })
        );
    ...
    editCouponsFade: function (basketItemsNode) {
      if (this.result.COUPON_LIST.length < 1) return;
    ...
    getCouponNode: function (coupon, active) {
      var couponName = BX.util.htmlspecialchars(coupon.text) || "",
        couponDesc =
          coupon.desc && coupon.desc.length
      ...
      return BX.create("STRONG", {
        ...
        children: active
          ? [
              //couponName || "",
              BX.create("SPAN", { props: { className: "bx-soa-coupon-item__text" }, text: couponName || "" }),
              // BX.create("SPAN", {
    ...
    addCoupon: function (coupon) {
      var couponListNodes = this.orderBlockNode.querySelectorAll(
        ".bx-soa-coupon:not(.bx-soa-coupon-item-fixed) .bx-soa-coupon-item"
      );

      for (var i = 0; i < couponListNodes.length; i++) {
        if (couponListNodes[i].querySelector('[data-coupon="' + BX.util.htmlspecialchars(coupon) + '"]')) break;
    ...
    removeCoupon: function (coupon) {
      var couponNodes = this.orderBlockNode.querySelectorAll(
          '[data-coupon="' + BX.util.htmlspecialchars(coupon) + '"]'
        ),
        i;
    ...
    editActiveRegionBlock: function (activeNodeMode) {
      ...
      if (this.initialized.region) {
        ...
        } else BX.cleanNode(regionContent);
    ...
    editFadeRegionBlock: function () {
      var regionContent = this.regionBlockNode.querySelector(".bx-soa-section-content"),
        newContent;
    ...
    editFadeRegionContent: function (node) {
      if (!node || !this.locationsInitialized) return;
      ...
      if (errorNode) node.appendChild(errorNode.cloneNode(true));
      ...
      if (this.regionBlockNode.getAttribute("data-visited") == "true") {
        ...
        } else BX.removeClass(this.regionBlockNode, "bx-step-error");
      }
    ...
    getSelectedPersonType: function () {
      ...
      if (personTypeLength == 1) {
        personTypeInput = this.propsBlockNode.querySelector("input[type=hidden][name=PERSON_TYPE]");
      } else if (personTypeLength == 2) {
        personTypeInput = this.propsBlockNode.querySelector("input[type=radio][name=PERSON_TYPE]:checked");
      } else {
        personTypeInput = this.propsBlockNode.querySelector("select[name=PERSON_TYPE] > option:checked");
      }
    ...
    getLocationString: function (node) {
      ...
      if (locationInputNode && locationInputNode.value && locationInputNode.value.length)
        ...
        for (i = locationSteps.length; i--; ) {
          if (locationSteps[i].innerHTML.indexOf("...") >= 0) continue;

          if (locationSteps[i].innerHTML.indexOf("---") >= 0) {
            altLoc = BX("altProperty");
            if (altLoc && altLoc.value.length) locationString += altLoc.value;

            continue;
          }

          if (locationString.length) locationString += ", ";

          locationString += locationSteps[i].innerHTML;
        }

        if (locationString.length == 0) locationString = BX.message("SOA_NOT_SPECIFIED");
      }
    ...
    getZipLocationInput: function (node) {
      ...
      if (zipProperty) {
        ...
        const $zipWrapper = BX.create("DIV", { props: { className: "col-md-12" }, children: [$zipInner] });
        node.appendChild($zipWrapper);
        /*node.appendChild(
          BX.create("input", {
            props: {
              id: "ZIP_PROPERTY_CHANGED",
      ...
      if (personTypesCount > 2) {
        ...
        for (i in this.result.PERSON_TYPE) {
          if (this.result.PERSON_TYPE.hasOwnProperty(i)) {
            ...
            node.appendChild(
              BX.create("DIV", {
                props: { className: "person-type" + (currentType.CHECKED == "Y" ? " active" : "") },
                children: [label],
              })
            );
        ...
        for (i in this.result.PERSON_TYPE)
          if (this.result.PERSON_TYPE.hasOwnProperty(i))
            node.appendChild(
              BX.create("INPUT", {
                props: { type: "hidden", name: "PERSON_TYPE", value: this.result.PERSON_TYPE[i].ID },
              })
            );
      }
    ...
    getProfilesControl: function (node) {
      ...
      if (profilesLength) {
        if (this.params.ALLOW_USER_PROFILES === "Y" && (profilesLength > 1 || this.params.ALLOW_NEW_PROFILE === "Y")) {
          this.regionBlockNotEmpty = true;
    ...
    editPaySystemBlock: function (active) {
      if (!this.paySystemBlockNode || !this.paySystemHiddenBlockNode || !this.result.PAY_SYSTEM) return;

      /*if (active) this.editActivePaySystemBlock(true);
      else this.editFadePaySystemBlock();
      */

      this.editActivePaySystemBlock(active);
    ...
    editActivePaySystemBlock: function (activeNodeMode) {
      ...
      if (this.result.PAY_SYSTEM.length === 1) {
        this.paySystemBlockNode.classList.add("hidden");
      } else {
        this.paySystemBlockNode.classList.remove("hidden");
      }
      ...
      if (this.initialized.paySystem) {
        ...
        if (!this.opened["bx-soa-paysystem"]) this.paySystemBlockNode.classList.add("bx-step-completed");
        ...
        /*if (this.params.SHOW_COUPONS_PAY_SYSTEM == "Y") {
          ...
          if (!this.couponsCustomInitialized) {
            var divs = BX.findChildren(BX("bx-soa-coupon"), { className: "bx-soa-section-content" });
            if (divs && divs.length) {
              BX.cleanNode(divs[0]);
              this.editCoupons(divs[0]);
              this.couponsCustomInitialized = true;
            }
          }
        }*/
      ...
      if (!activeNodeMode || !this.initialized.paySystem) {
        this.getPaySystemCompactInfo();
      }
    },
    ...
    getPaySystemCompactInfo: function () {
      ...
      if (currentPaySystem) {
        ...
        const $paymentLabel = BX.create("div", {
          props: { className: "bx-compact-title" },
          html: currentPaySystem["NAME"],
        });
        childrenBlocks.push($paymentLabel);

        if (this.params.SHOW_IMAGES == "Y") {
          const $paymentImage = this.getImageNode(currentPaySystem, "PAY_SYSTEM");
          childrenBlocks.push($paymentImage);
        }
    ...
    editFadePaySystemBlock: function () {
      ...
      if (this.initialized.paySystem) {
        ...
        this.opened["bx-soa-paysystem"] = false;
      } else {
    ...
    editPaySystemItems: function (paySystemNode) {
      ...
      var paySystemItemsDescription = BX.create("DIV", {
        props: { className: "col-sm-12 bx-soa-pp-company-description" },
      });
      ...
      for (i = 0; i < this.paySystemPagination.currentPage.length; i++) {
        ...
        paySystemItemsContainer.appendChild(paySystemItemNode);
        if (this.paySystemPagination.currentPage[i].CHECKED == "Y") {
          paySystemItemsDescription.innerHTML = this.paySystemPagination.currentPage[i].DESCRIPTION;
        }
      }
      ...
      // paySystemNode.appendChild(paySystemItemsDescription);
    },

    getImageNode: function (item, mode) {
      let logotype, logoNode;

      let isDelivery = mode === "DELIVERY";

      logoNode = BX.create("DIV", { props: { className: "bx-soa-pp-company-image" } });
      logotype = this.getImageSources(item, isDelivery ? "LOGOTIP" : "PSA_LOGOTIP");
      if (logotype && logotype.src_2x) {
        logoNode.setAttribute(
          "style",
          "background-image: url(" +
        ...
        logotype = (logotype && logotype.src_1x) || (isDelivery ? this.defaultDeliveryLogo : this.defaultPaySystemLogo);
        logoNode.setAttribute("style", "background-image: url(" + logotype + ");");
      }
    ...
    createPaySystemItem: function (item) {
      ...
      if (this.params.SHOW_IMAGES == "Y") {
        logoNode = this.getImageNode(item, "PAY_SYSTEM");
      }
      ...
      labelNodes = [
        BX.create("INPUT", {
          props: {
            ...
            // type: "checkbox",
            type: "radio",
            className: "bx-soa-pp-company-checkbox form-radiobox__input",
            value: paySystemId,
            checked: checked,
          },
        }),
        BX.create("LABEL", {
          props: {
            ...
            className: "bx-soa-pp-company-label properties__title char_name form-radiobox__label ",
            lang: "ru",
          },
          html:
            "<span>" +
            (this.params.SHOW_PAY_SYSTEM_LIST_NAMES != "N" ? item.NAME : item.OWN_NAME) +
            "</span>" +
            "<span class='form-radiobox__box'></span>" +
            (this.uniqueText(item.DESCRIPTION)
              ? "<div class='hint'>" +
                "<span class='hint__icon bordered rounded bg-theme-hover border-theme-hover'><i>?</i></span>" +
                "<div class='tooltip'>" +
                item.DESCRIPTION +
                "</div>" +
                "</div>"
      ...
      label = BX.create("DIV", {
        props: {
          className: "bx-soa-pp-company-graf-container properties form-radiobox",
        },
        children: labelNodes,
      });
      ...
      */
      ...
      itemInnerNode = BX.create("DIV", {
        props: {
          className: "bx-soa-pp-company-inner filter radio bordered rounded3",
        },
        children: [label, logoNode, injectNode],
      });
    ...
    editPaySystemInfo: function (paySystemNode) {
      ...
      var injectNodes = BX.findChildren(this.paySystemBlockNode, { className: "bx-soa-pp-company-inject" }, true);
      if (injectNodes && injectNodes.length) {
        for (var i = 0; i < injectNodes.length; ++i) {
          BX.addClass(injectNodes[i], "hidden");
          BX.cleanNode(injectNodes[i]);
        }
      }

      if (currentPaySystem) {
        var paySystemNode = BX.findChildren(
          this.paySystemBlockNode,
          { className: "bx-soa-pp-company", attribute: { "data-id": currentPaySystem.ID } },
          true
        );
    ...
    getInnerPaySystem: function () {
      ...
      label = BX.create("LABEL", {
        props: {
          className:
            "bx-soa-pp-company-subTitle form-checkbox__label form-checkbox__label--sm" + (isSelected ? " checked" : ""),
          htmlFor: "PAY_CURRENT_ACCOUNT",
        },
        children: [
          BX.create("span", {
            props: {
              className: "form-checkbox__box form-checkbox__box--static",
            },
          }),
          paySystem.NAME,
        ],
      ...
      hiddenInput = BX.create("INPUT", {
        props: {
          type: "checkbox",
          className: "bx-soa-pp-company-checkbox form-checkbox__input",
          name: "PAY_CURRENT_ACCOUNT",
      ...
      // balanceString = BX.util.htmlspecialchars(
      //   this.params.MESS_INNER_PS_BALANCE + " " + this.result.CURRENT_BUDGET_FORMATED
      // );
      // balanceString = this.params.MESS_INNER_PS_BALANCE + " " + this.result.CURRENT_BUDGET_FORMATED;
      // label.setAttribute("title", balanceString);
      ...
      htmlString =
        BX.message("INNER_PS_BALANCE_DEFAULT") + ' <b class="wsnw">' + this.result.CURRENT_BUDGET_FORMATED + "</b>";
    ...
    editFadePaySystemContent: function (node) {
      ...
      if (errorNode) node.appendChild(errorNode.cloneNode(true));
      else this.getErrorContainer(node);

      if (warningNode && warningNode.innerHTML) node.appendChild(warningNode.cloneNode(true));
      ...
      // if (!addedHtml.length)
      if (!this.isSelectedInnerPayment() && !(selectedPaySystem && selectedPaySystem.NAME))
    ...
    getSelectedPaySystem: function () {
      ...
      if (!paySystemCheckbox)
        paySystemCheckbox = this.paySystemHiddenBlockNode.querySelector("input[name=PAY_SYSTEM_ID]:checked");
    ...
    isSelectedInnerPayment: function () {
      ...
      if (!innerPaySystemCheckbox)
        innerPaySystemCheckbox = this.paySystemHiddenBlockNode.querySelector("input[name=PAY_CURRENT_ACCOUNT]");
    ...
    selectPaySystem: function (event) {
      ...
      var innerPsAction = BX.hasClass(target, "bx-soa-pp-inner-ps")
          ? target
          : BX.findParent(target, { className: "bx-soa-pp-inner-ps" }),
      ...
      if (innerPsAction) {
        if (target.nodeName == "INPUT") innerPaySystemCheckbox.checked = !innerPaySystemCheckbox.checked;
        ...
        if (innerPaySystemCheckbox && innerPaySystemCheckbox.checked && fullPayFromInnerPaySystem) {
          ...
          actionInput = actionSection.querySelector("input.bx-soa-pp-company-checkbox");
          ...
          if (selectedSection) {
            ...
            selectedSection.querySelector("input.bx-soa-pp-company-checkbox").checked = false;
          }
        }
      }
    ...
    editDeliveryBlock: function (active) {
      ...
      */
      this.editActiveDeliveryBlock(active);
    ...
    editActiveDeliveryBlock: function (activeNodeMode) {
      ...
      var node = this.deliveryBlockNode,
        deliveryContent,
        deliveryNode;

      /*if (this.result.DELIVERY.length === 1) {
        this.deliveryBlockNode.classList.add("hidden");
      }*/

      if (this.result.IS_AUTHORIZED && !this.opened[this.deliveryBlockNode.id]) {
        this.opened[this.deliveryBlockNode.id] = true;
        BX.removeClass(this.deliveryBlockNode, "bx-selected");
        // BX.removeClass(this.deliveryBlockNode, "bx-step-completed");
      }
      ...
      if (this.initialized.delivery) {
        ...
        if (!this.opened[this.deliveryBlockNode.id]) this.deliveryBlockNode.classList.add("bx-step-completed");
        ...
        this.getDeliveryLocationInput(node.querySelector(".bx-soa-section-location"));
        ...
        this.showPickUp(deliveryNode);

        this.getZipLocationInput(deliveryNode);
        if (this.params.HIDE_ORDER_DESCRIPTION !== "Y") {
          this.editPropsComment(deliveryNode);
        }
        this.getBlockFooter(deliveryContent);
      }
      if (!activeNodeMode || !this.initialized.delivery) {
        this.getDeliveryCompactInfo();
      }
    },
    getDeliveryCompactInfo: function () {
      ...
      let currentDelivery = this.getSelectedDelivery();
        ...
        const childrenBlocks = [];
        const $deliveryLabel = BX.create("div", {
          props: { className: "bx-compact-title" },
          html: currentDelivery["NAME"] ?? BX.message('BX_SOA_NO_DELIVERY'),
        });
        childrenBlocks.push($deliveryLabel);

      if (currentDelivery) {
        if (this.params.SHOW_IMAGES == "Y") {
          const $deliveryImage = this.getImageNode(currentDelivery, "DELIVERY");
          childrenBlocks.push($deliveryImage);
          const separateNode = BX.create("DIV", {
            props: { className: "bordered-bottom bx-soa-grey-separate-node bx-soa-grey-separate-node--mb" },
          });
          childrenBlocks.push(separateNode);
        }
        ...
        const $location = BX.create("div", {
          props: { className: "bx-compact-prop" },
          html: this.getDerivedBlock(
            BX.message("TITLE_SOA_DELIVERY_CITY"),
            '<span class="destination_value">' +
              this.deliveryBlockNode.querySelector(".destination_value").textContent +
              "</span>"
          ),
        });
        ...
        if (currentDelivery["PRICE_FORMATED"]) {
          const $price = BX.create("div", {
            props: { className: "bx-compact-prop" },
            html: this.getDerivedBlock(
              BX.message("TITLE_SOA_DELIVERY_COST"),
              currentDelivery["PRICE"] > 0
                ? typeof currentDelivery["DELIVERY_DISCOUNT_PRICE"] !== "undefined"
                  ? currentDelivery["DELIVERY_DISCOUNT_PRICE"] > 0
                    ? currentDelivery["DELIVERY_DISCOUNT_PRICE_FORMATED"]
                    : BX.message("PRICE_FREE_DEFAULT")
                  : currentDelivery["PRICE_FORMATED"]
                : BX.message("PRICE_FREE_DEFAULT")
            ),
          });
    ...
    getDerivedBlock: function (name, value) {
      return (
        '<div class="bx-compact__prop-title">' +
        name +
        "</div>" +
        '<div class="bx-compact__prop-value">' +
        value +
        "</div>"
      );
    },
    ...
    showPropInDelivery: function (propCode, node, title) {
      ...
      if (title) {
        titleNode = BX.create("DIV", {
          props: {
            className: "bx-soa-title-subblock",
          },
          text: title,
        });
      }
      ...
      const $propInner = BX.create("DIV", {
        props: {
          className: "bx-soa-pp-company-item " + propCode,
        },
        children: [titleNode],
      });
      const $prop = BX.create("DIV", {
        props: {
          className: "col-md-12 bx-soa-extraprops group-without-margin",
        },
        children: [$propInner],
      });
      let isAddProp = false;
    ...
    showPickUp: function (node) {
      const $pickup = BX.create("div", {
        ...
        children: [
          BX.create("div", {
            props: {
              className: "bx-soa-section-content bx-soa-pp-company-item",
            },
          }),
        ],
      });
      node.appendChild($pickup);
      this.pickUpBlockNode = BX("bx-soa-pickup");
    },
    ...
    editDeliveryInfo: function (deliveryNode) {
      ...
      title = BX.create("DIV", {
        props: { className: "bx-soa-pp-company-block" },
        children: [
          BX.create("DIV", { props: { className: "bx-soa-pp-company-desc" }, html: "" }),
          //   BX.create("DIV", { props: { className: "bx-soa-pp-company-desc" }, html: "" }),
      ...
      if (extraServices.length) {
        ...
        deliveryNode.appendChild(
          BX.create("DIV", { props: { className: "col-md-12 bx-soa-extraservices" }, children: [extraServicesNode] })
        );
        extraServicesNode = BX.create("DIV", {
          props: { className: "bx-soa-pp-company-block bx-soa-pp-company-block__extra-services" },
          children: extraServices,
        });
      }
      ...
      var injectNodes = BX.findChildren(this.deliveryBlockNode, { className: "bx-soa-pp-company-inject" }, true);
      if (injectNodes && injectNodes.length) {
        for (var i = 0; i < injectNodes.length; ++i) {
          BX.addClass(injectNodes[i], "hidden");
          BX.cleanNode(injectNodes[i]);
      ...
      var deliveryNode = BX.findChildren(
        this.deliveryBlockNode,
        { className: "bx-soa-pp-company", attribute: { "data-id": currentDelivery.ID } },
        true
      );
      if (deliveryNode && deliveryNode.length) {
        var injectNode = BX.findChildren(deliveryNode[0], { className: "bx-soa-pp-company-inject" }, true);
        if (injectNode && injectNode.length) {
          BX.cleanNode(injectNode[0]);
          injectNode[0].appendChild(deliveryInfoContainer);
          BX.removeClass(injectNode[0], "hidden");
        }
      }
    ...
    getDeliveryPriceNodes: function (delivery) {
      ...
      if (
        typeof delivery.DELIVERY_DISCOUNT_PRICE !== "undefined" &&
    ...
    getDeliveryExtraServices: function (delivery) {
      const nodeDeliveryLocationWarning = this.deliveryBlockNode.querySelector(".bx-soa-delivery-warning");

      if (!delivery) {
        if (!nodeDeliveryLocationWarning) {
          const nodeDeliveryBlockContent = this.deliveryBlockNode.querySelector('.bx-soa-section-content');
          const nodeDelivertBlockContentFields = nodeDeliveryBlockContent.querySelector('.bx-soa-pp');

          nodeDelivertBlockContentFields.style.display = 'none';
          nodeDeliveryBlockContent.removeAttribute('style');

          nodeDeliveryBlockContent.appendChild(
            BX.create("div", {
              props: {
                className: "bx-soa-delivery-warning",
              },
              children: [
                BX.create("div", {
                  props: {
                  ...
                  text: BX.message("SOA_DELIVERY_LOCATION_INVALID"),
                }),
              ],
            })
          );
      ...
      if (nodeDeliveryLocationWarning) {
        nodeDeliveryLocationWarning.remove();
      }
      ...
      var extraServices = [],
        ...
        serviceName,
        input;
      if (delivery.EXTRA_SERVICES.length) {
        extraServices.push(
          BX.create("div", {
      ...
      for (i in delivery.EXTRA_SERVICES) {
        if (!delivery.EXTRA_SERVICES.hasOwnProperty(i)) continue;

        currentService = delivery.EXTRA_SERVICES[i];

        if (!currentService.canUserEditValue) continue;
        ...
        if (currentService.editControl.indexOf("this.checked") == -1) {
          ...
          if (!input) input = serviceNode.querySelector("select");

          if (input) BX.addClass(input, "form-control");
        } else {
          let serviceID = currentService.editControl.match(/name="(.+?)"/);
          serviceNode = BX.create("DIV", {
            props: { className: "form-group filter label_block" },
            children: [
              currentService.editControl.replace(
                "onchange",
                'id="' + serviceID[1] + '" class="form-checkbox__input" onchange'
              ),
              BX.create("LABEL", {
                props: {
                  htmlFor: serviceID[1],
    ...
    editFadeDeliveryBlock: function () {
      var deliveryContent = this.deliveryBlockNode.querySelector(".bx-soa-section-content"),
        newContent;

      if (this.initialized.delivery) {
        // this.deliveryHiddenBlockNode.appendChild(deliveryContent);
        this.opened["bx-soa-delivery"] = false;
      } else {
        this.editActiveDeliveryBlock(false);
    ...
    createDeliveryItem: function (item) {
      //let bShowFullDesc = item.DESCRIPTION.indexOf("sdek_pvz_btn") != -1 || item.DESCRIPTION.indexOf("russianpost_link") != -1 || item.DESCRIPTION.indexOf("show_full_delivery_description") != -1 ;
      var checked = item.CHECKED == "Y",
        ...
        labelNodes = [
          ...
          BX.create("LABEL", {
            ...
            html:
              "<span>" +
              (this.params.SHOW_DELIVERY_PARENT_NAMES != "N" ? item.NAME : item.OWN_NAME) +
              "</span>" +
              "<span class='form-radiobox__box'></span>" +
              (this.uniqueText(item.DESCRIPTION) && !checked
                ? "<div class='hint '>" +
                  "<span class='hint__icon bordered rounded bg-theme-hover border-theme-hover'><i>?</i></span>" +
                  "<div class='tooltip'>" +
                  item.DESCRIPTION +
      ...
      if (this.params.SHOW_IMAGES == "Y") {
        logoNode = this.getImageNode(item, "DELIVERY");
      }

      var bHasPrice = false;
      if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== "undefined") {
        bHasPrice = true;
        costNode = BX.create("DIV", {
          props: { className: "bx-soa-pp-delivery-cost" },
          html:
            '<div class="bx-soa-pp-list-termin" lang="ru">' +
            this.params.MESS_PRICE +
            ":" +
            "</div>" +
            '<div class="bx-soa-pp-list-description" lang="ru">' +
            (item.PRICE > 0
      ...
      label = BX.create("DIV", {
        props: {
          className:
            "bx-soa-pp-company-graf-container properties form-radiobox " +
      ...
      if (bHasPeriod) {
        periodNode = BX.create("DIV", {
          props: { className: "bx-soa-pp-delivery-period" },
          html:
            '<div class="bx-soa-pp-list-termin" lang="ru">' +
            this.params.MESS_PERIOD +
            ":" +
            "</div>" +
            '<div class="bx-soa-pp-list-description" lang="ru">' +
            item.PERIOD_TEXT +
      ...
      if (bHasCalculate) {
        ...
        calculateNode.setAttribute("lang", "ru");
      }

      if (logoNode && checked && (item.DESCRIPTION || bHasPeriod || bHasPeriod || bHasCalculate)) {
        separateNode = BX.create("DIV", {
          props: { className: "bordered-bottom bx-soa-grey-separate-node" },
        });
      }
      ...
      itemNode = BX.create("DIV", {
        props: {
          className:
            ...
            (bHasPrice ? " bx-soa-pp-company--hasprice" : "") +
            (bHasPeriod ? " bx-soa-pp-company--hasperiod" : "") +
            (bHasCalculate ? " bx-soa-pp-company--hascalculate" : "") +
            " col-sm-6 col-xs-12",
        },
        children: [itemInnerNode],
      });
    ...
    editFadeDeliveryContent: function (node) {
      ...
      if (errorNode && errorNode.innerHTML) node.appendChild(errorNode.cloneNode(true));
      else this.getErrorContainer(node);
      ...
      if (selectedDelivery && selectedDelivery.NAME) {
        ...
        // 	})
        // );
      } else node.appendChild(BX.create("STRONG", { text: BX.message("SOA_DELIVERY_SELECT_ERROR") }));
    ...
    selectDelivery: function (event) {
      ...
      var target = event.target || event.srcElement,
        ...
        actionSection = BX.hasClass(target, "bx-soa-pp-company-item")
          ? target
          : BX.findParent(target, { className: "bx-soa-pp-company-item" }),
      ...
      if (!actionCompany) {
        if (BX.hasClass(actionSection, "bx-selected")) {
          if (target.nodeName == "INPUT" || target.nodeName == "A" || target.nodeName == "LABEL") {
            return;
          } else {
            return BX.PreventDefault(event);
          }
    ...
    getSelectedDelivery: function () {
      ...
      if (!deliveryCheckbox)
        deliveryCheckbox = this.deliveryHiddenBlockNode.querySelector("input[type=radio][name=DELIVERY_ID]:checked");
    ...
    activatePickUp: function (deliveryName) {
      if (!this.pickUpBlockNode || !this.pickUpHiddenBlockNode) return;

      this.pickUpBlockNode.style.display = "";

      if (BX.hasClass(this.pickUpBlockNode, "bx-active")) return;
    ...
    editPickUpBlock: function (active) {
      ...
      if (active) this.editActivePickUpBlock(true);
      else this.editFadePickUpBlock();
    ...
    editActivePickUpBlock: function (activeNodeMode) {
      ...
      if (this.initialized.pickup) {
        ...
        if (this.params.SHOW_NEAREST_PICKUP === "Y" && this.maps && !this.maps.maxWaitTimeExpired) {
        ...
        let notShowMapClass = this.params.SHOW_PICKUP_MAP === "Y" ? "" : "pickup-wrapper--no-map";
        var pickUpListOuter = BX.create("DIV", {
          props: { className: "bx-soa-pickup-list-outer-wrap scrollbar" },
        });
    ...
    editFadePickUpBlock: function () {
      var pickUpContent = this.pickUpBlockNode.querySelector(".bx-soa-section-content"),
        newContent;
    ...
    editFadePickUpContent: function (pickUpContainer) {
      var selectedPickUp = this.getSelectedPickUp(),
        html = "",
        logotype,
        imgSrc;
      ...
      if (selectedPickUp) {
        ...
        if (selectedPickUp.ADDRESS)
          html +=
            "<br><strong>" +
            BX.message("SOA_PICKUP_ADDRESS") +
            ":</strong> " +
            BX.util.htmlspecialchars(selectedPickUp.ADDRESS);

        if (selectedPickUp.PHONE)
          html +=
            "<br><strong>" +
            BX.message("SOA_PICKUP_PHONE") +
            ":</strong> " +
            BX.util.htmlspecialchars(selectedPickUp.PHONE);

        if (selectedPickUp.SCHEDULE)
          html +=
            "<br><strong>" +
            BX.message("SOA_PICKUP_WORK") +
            ":</strong> " +
            BX.util.htmlspecialchars(selectedPickUp.SCHEDULE);

        if (selectedPickUp.DESCRIPTION)
          html +=
            "<br><strong>" +
            BX.message("SOA_PICKUP_DESC") +
            ":</strong> " +
            BX.util.htmlspecialchars(selectedPickUp.DESCRIPTION);
    ...
    getPickUpInfoArray: function (storeIds) {
      ...
      for (i = 0; i < storeIds.length; i++)
        if (this.result.STORE_LIST[storeIds[i]]) arr.push(this.result.STORE_LIST[storeIds[i]]);
    ...
    geoLocationSuccessCallback: function (result) {
      ...
      if (activeStores && activeStores.length >= this.options.pickUpMap.minToShowNearestBlock && result) {
        ...
        if (this.params.SHOW_PICKUP_MAP === "Y" && BX("pickUpMap")) {
          ...
          if (activeStores && activeStores.length) {
            ...
            if (this.params.SHOW_NEAREST_PICKUP === "Y") {
              this.maps.showNearestPickups(
                BX.proxy(this.geoLocationSuccessCallback, this),
                BX.proxy(this.geoLocationFailCallback, this)
              );
            }

            this.maps.buildBalloons(activeStores);

            // window.map = this.maps.pickUpMap;
            setTimeout(
              BX.proxy(function () {
                BX("pickUpMap").appendChild(
                  BX.create("DIV", {
                    props: { className: "yandex-map__mobile-opener" },
                    events: {
                      click: BX.proxy(function (e) {
                        const $pickup = BX("pickUpMap");
                        if (!isFullscreen($pickup)) {
                          if (this.maps.pickUpMap.container) this.maps.pickUpMap.container.enterFullscreen();
                          if (this.maps.pickUpMap.getDiv) this.maps.pickUpMap.getDiv().requestFullscreen();
                        } else {
                        ...
                        document.onwebkitfullscreenchange =
                          ...
                          document.onfullscreenchange =
                            function () {
                              if (isFullscreen($pickup)) {
                                $pickup.classList.add("is-fullscreen");
                              } else {
                                $pickup.classList.remove("is-fullscreen");
                              }
                            };
    ...
    editPickUpMap: function (pickUpContent) {
      pickUpContent.appendChild(
        BX.create("DIV", {
          props: {
            id: "pickUpMap",
            className: "bx-yandex-view-layout",
          },
          style: { width: "100%" },
        })
      );
    ...
    editPickUpList: function (isNew) {
      ...
      if (!recommendList) recommendList = this.pickUpHiddenBlockNode.querySelector(".bx-soa-pickup-list.recommend");
      ...
      if (!!isNew) {
        container = this.pickUpHiddenBlockNode.querySelector(".bx_soa_pickup .bx-soa-pickup-list-outer-wrap");
        if (!container) container = this.pickUpBlockNode.querySelector(".bx-soa-pp-company-item");
        ...
        container.insertBefore(
          ...
          this.pickUpBlockNode.querySelector(".bx_soa_pickup")
        );
        container.querySelector(".bx_soa_pickup .bx-soa-pickup-list-outer-wrap").appendChild(pickUpList);
      } else {
    ...
    getStoreInfoHtml: function (currentStore) {
      ...
      if (currentStore.PHONE)
        html +=
          ...
          BX.message("SOA_PICKUP_PHONE") +
          ": " +
          BX.util.htmlspecialchars(currentStore.PHONE) +
          "</div>";
      ...
      if (currentStore.SCHEDULE)
        html +=
          ...
          BX.message("SOA_PICKUP_WORK") +
          ": " +
          BX.util.htmlspecialchars(currentStore.SCHEDULE) +
          "</div>";
    ...
    createPickUpItem: function (currentStore, options) {
      ...
      var imgClassName = "bx-soa-pickup-l-item-props",
      ...
      storeNode = BX.create("DIV", {
        ...
        children: [
          BX.create("DIV", {
            props: {
              className: "bx-soa-pickup-item-info",
            },
            children: [
              BX.create("DIV", {
                ...
                children: options.distance
                  ? [
                      BX.util.htmlspecialchars(titleStore + " " + currentStore.ADDRESS),
              ...
              BX.create("DIV", {
                ...
                children: [BX.create("DIV", { props: { className: "bx-soa-pickup-l-item-desc font_13" }, html: html })],
              }),
            ],
          }),
          BX.create("DIV", {
            ...
            children: [
              BX.create("A", {
                props: { href: "javascript:;", className: "btn btn-sm btn-default " },
    ...
    editPickUpRecommendList: function (geoLocation) {
      ...
      container = this.pickUpHiddenBlockNode.querySelector(".bx_soa_pickup .bx-soa-pickup-list-outer-wrap");
      if (!container) {
        container = this.pickUpBlockNode.querySelector(".bx_soa_pickup .bx-soa-pickup-list-outer-wrap");
      }
    ...
    selectStore: function (event) {
      ...
      if (storeItem && storeInput) {
        ...
        if (selectedPickUp) {
          selectedPickUp.querySelector(".bx-soa-pickup-l-item-btn .btn").innerText = this.params.MESS_SELECT_PICKUP;
        ...
        storeItem.querySelector(".bx-soa-pickup-l-item-btn .btn").innerText = this.params.MESS_SELECTED_PICKUP;
        ...
        sortFunc = function (a, b) {
          ...
          if (sort === 0) {
            return a.OWN_NAME.toLowerCase() > b.OWN_NAME.toLowerCase()
              ? 1
              : a.OWN_NAME.toLowerCase() < b.OWN_NAME.toLowerCase()
              ? -1
              : 0;
          } else {
            return sort;
    ...
    editPropsBlock: function (active) {
      ...
      if (active) this.editActivePropsBlock(true);
      else this.editFadePropsBlock();
    ...
    editActivePropsBlock: function (activeNodeMode) {
      ...
      if (this.opened[this.propsBlockNode.id]) {
        this.propsBlockNode.classList.remove("bx-step-completed");
        this.propsBlockNode.classList.remove("bx-selected");
      }
      ...
      if (this.initialized.props) {
        ...
        if (!this.result.IS_AUTHORIZED && !this.opened[this.propsBlockNode.id]) {
          this.opened[this.propsBlockNode.id] = true;
        }

        if (!this.opened[this.propsBlockNode.id]) this.propsBlockNode.classList.add("bx-step-completed");
        ...
        this.showPersonType();
        // this.getProfilesControl(propsContent);
        ...
        this.showSaveProfile(propsContent);
      ...
      // if (!activeNodeMode || !this.initialized.props) {
      this.getPropsCompactInfo();

      if (this.hasUserProfiles() || !this.result.IS_AUTHORIZED) {
        this.showFirstProps();
      } else {
        // when authorized and without profiles
        setTimeout(
          BX.delegate(function () {
            this.showFirstProps();
          }, this),
    ...
    showFirstProps: function () {
      if (this.checkEmptyProps()) {
        this.opened[this.propsBlockNode.id] = true;
        this.propsBlockNode.classList.remove("bx-step-completed");
        this.propsBlockNode.classList.remove("bx-selected");
      } else if (this.opened[this.propsBlockNode.id] && !this.opened["edit_profile"]) {
        this.opened[this.propsBlockNode.id] = false;
        this.propsBlockNode.classList.add("bx-step-completed");
      }
    },
    checkEmptyProps: function () {
      return this.isValidPropertiesBlock(true, this.propsBlockNode, true).length;
    },
        ...
        BX.create("DIV", {
          ...
          children: [
            BX.create("DIV", {
              ...
              html: BX.message("TITLE_SOA_SAVE_INFO_SECTION"),
              events: {
                click: BX.proxy(this.saveProfile, this),
              },
            }),
          ],
        })
    ...
    saveProfile: function () {
      ...
      BX.cleanNode(this.propsBlockNode.querySelector(".alert.alert-danger"));
      BX.removeClass(this.propsBlockNode, "bx-step-error");

      if (this.hasUserProfiles()) {
        if (!this.startLoader()) return;
        ...
        BX.ajax({
          ...
          onsuccess: BX.delegate(function (result) {
            ...
            if (result.profileID) {
              this.opened[this.propsBlockNode.id] = false;
              this.opened["edit_profile"] = false;

              this.showTempProfileInput(result);

              this.sendRequest("", { "change-profile": "Y" });
            }
        ...
        this.opened[this.propsBlockNode.id] = false;
        this.fade(this.propsBlockNode, this.propsBlockNode);
    ...
    showTempProfileInput: function (result) {
      this.propsBlockNode.querySelector(".profiles-content").appendChild(
        BX.create("INPUT", {
          props: {
            type: "radio",
    ...
    getPropsCompactInfo: function () {
      ...
      BX.cleanNode($profile);

      /*setTimeout(
        BX.proxy(function () {*/
      this.showInfoFromProps($profile);
      /*}, this),
        100
      );*/
    },
    ...
    showInfoFromProps: function (node) {
      ...
      if (this.hasUserProfiles() && this.params.ALLOW_USER_PROFILES === "Y") {
        for (let key in this.result.USER_PROFILES) {
          content.unshift(this.showPropItem(this.result.USER_PROFILES[key]));
        }

        if (this.params.ALLOW_NEW_PROFILE === "Y") {
          newProfile = BX.create("div", {
            props: { className: "col-sm-12" },
            children: [
              BX.create("label", {
                props: { className: "btn btn-default add-profile", htmlFor: "ID_PROFILE_ID_0" },
                ...
                children: [
                  ...
                  BX.create("span", {
                    text: BX.message("ADD_NEW_PROFILE"),
                  }),
                ],
                events: {
                  click: BX.delegate(this.changeProfile, this),
                },
              }),
            ],
          });
        ...
        content.push(this.showPropItem());
      }
      node.appendChild(
        BX.create("div", {
          props: { className: "bx-soa-pp row" },
          children: [
            BX.create("div", { props: { className: "col-sm-12 bx-soa-pp-item-container" }, children: content }),
            newProfile,
            profileChangeInput,
          ],
        })
    ...
    changeProfile: function () {
      this.opened[this.propsBlockNode.id] = true;
      this.opened["edit_profile"] = true;
      BX("ID_PROFILE_ID_0").setAttribute("type", "hidden");
      BX("profile_change").value = "Y";
      this.sendRequest();
    },
    showPropItem: function (profile) {
      ...
      const propForName = this.getPropByFilter({ USER_PROPS: "Y", IS_PROFILE_NAME: "Y" });
      // let profileValue = propForName["VALUE"].toString() || BX.message("NO_PROFILE_NAME");
      let profileValue = this.getInputValueForProp(propForName) || BX.message("NO_PROFILE_NAME");
      if (profile && profile["NAME"]) {
        profileValue = profile["NAME"];
      }
      // const profileValue = this.getInputValueForProp(propForName) || BX.message("NO_PROFILE_NAME");
      const checked = profile ? profile.CHECKED : "Y";
      ...
      const labelNodes = [
        ...
        BX.create("LABEL", {
          props: {
            ...
            className: "bx-soa-pp-company-label properties__value bx-soa-block-title form-radiobox__label",
            lang: "ru",
          },
          html: "<span>" + profileValue + "</span>" + "<span class='form-radiobox__box'></span>",
        }),
      ];
      ...
      let isBottomProps = false;
      const bottomProps = [];

      const propForEmail = this.getPropByFilter({ USER_PROPS: "Y", IS_EMAIL: "Y" });
      let emailValue = this.getInputValueForProp(propForEmail);
      if (
        profile &&
        this.result.USER_PROFILES_PROPS &&
        this.result.USER_PROFILES_PROPS[profile["ID"]] &&
        this.result.USER_PROFILES_PROPS[profile["ID"]]["EMAIL"]
      ) {
        emailValue = this.result.USER_PROFILES_PROPS[profile["ID"]]["EMAIL"];
      }
      if (emailValue) {
        const email = BX.create("DIV", {
          props: {
            className: "bx-soa-pp-company-label properties__value bx-soa-email",
          },
          html: "<span>" + emailValue + "</span>",
        });
        bottomProps.push(email);
        isBottomProps = true;
      }

      const propForPhone = this.getPropByFilter({ USER_PROPS: "Y", IS_PHONE: "Y" });
      let phoneValue = this.getInputValueForProp(propForPhone);
      if (
        profile &&
        this.result.USER_PROFILES_PROPS &&
        this.result.USER_PROFILES_PROPS[profile["ID"]] &&
        this.result.USER_PROFILES_PROPS[profile["ID"]]["PHONE"]
      ) {
        phoneValue = this.result.USER_PROFILES_PROPS[profile["ID"]]["PHONE"];
      }
      if (phoneValue) {
        const phone = BX.create("DIV", {
          props: {
            className: "bx-soa-pp-company-label properties__value bx-soa-phone",
          },
          html: "<span>" + phoneValue + "</span>",
        });
        bottomProps.push(phone);
        isBottomProps = true;
      }

      let changeProfile;
      if (checked === "Y") {
        changeProfile = BX.create("DIV", {
          props: {
            className: "bx-soa-inner",
          },
          html:
            "<span class='bx-soa-change-profile font_13'>" + BX.message("TITLE_SOA_CHANGE_INFO_SECTION") + "</span>",
      ...
      if (isBottomProps) {
        const itemBottomProps = BX.create("DIV", {
          props: {
            className: "bx-soa-pp-company-props",
          },
          children: [BX.create("div", { props: { className: "bx-soa-inner" }, children: bottomProps }), changeProfile],
        });
        innerInfo.push(itemBottomProps);
      } else {
        innerInfo.push(changeProfile);
      }

      const itemInnerNode = BX.create("DIV", {
        props: {
          className: "bx-soa-pp-company-inner filter radio bordered rounded3" + (checked ? " active" : ""),
        },
        children: innerInfo,
      });
      ...
      const content = BX.create("div", {
        props: {
          className: "bx-soa-pp-company bx-soa-pp-company-item col-sm-6 col-xs-12",
        },
        children: [itemInnerNode],
      });
    ...
    getInputValueForProp: function (prop) {
      if (prop) {
        ...
        if ($node && $node.value && $node.value.length > 1) {
    ...
    getPropByFilter: function (filter) {
      if (this.result.ORDER_PROP && this.result.ORDER_PROP.properties && filter) {
        return this.result.ORDER_PROP.properties.find(function (item) {
    ...
    showPersonType: function () {
      ...
      this.getPersonTypeControl($profile);
    },

    editFadePropsBlock: function () {
      var propsContent = this.propsBlockNode.querySelector(".bx-soa-section-content"),
        newContent;
    ...
    editPropsItems: function (propsNode) {
      ...
      var propsItemsContainer,
        propsInnerWrapper = BX.create("div", { props: { className: "row row-props group-without-margin" } }),
        group,
        property,
        groupIterator = this.propertyCollection.getGroupIterator(),
        propsIterator;
    ...
    getPropertyRowNode: function (property, propsItemsContainer, disabled, with_column) {
      var propsItemNode = BX.create("DIV"),
        ...
        label;
      let bHasColumn = with_column === undefined ? true : with_column;

      if (disabled) {
        propsItemNode.innerHTML = "<strong>" + BX.util.htmlspecialchars(property.getName()) + ":</strong> ";
      } else {
        BX.addClass(propsItemNode, "bx-soa-customer-field form-group");
        if (bHasColumn) {
          BX.addClass(propsItemNode, "col-sm-6");
        }
        ...
        if (disabled) {
          ...
          if (propRow) {
            ...
            for (i = 0; i < propNodes.length; i++) {
              ...
              values.push(
                locationString.length ? BX.util.htmlspecialchars(locationString) : BX.message("SOA_NOT_SELECTED")
              );
            }
          }
    ...
    addLocationProperty: function (e) {
      ...
      if (BX.hasClass(lastProp, "bx-soa-loc")) {
        if (this.locationsTemplate == "search") {
          ...
          if (input) index = parseInt(input.name.substring(input.name.indexOf("[") + 1, input.name.indexOf("]"))) + 1;
        } else {
      ...
      if (this.cleanLocations[propId]) {
        insertedLoc = BX.create("DIV", {
          ...
          html: this.cleanLocations[propId].HTML.split("#key#")
            .join(index)
            .replace(/sls-\d{5}/g, prefix + randomStr),
        });
        ...
        for (k in this.cleanLocations[propId].SCRIPT)
          if (this.cleanLocations[propId].SCRIPT.hasOwnProperty(k))
            BX.evalGlobal(
              this.cleanLocations[propId].SCRIPT[k].JS.split("_key__")
                .join("_" + index)
                .replace(/sls-\d{5}/g, prefix + randomStr)
            );
      ...
      if (disabled) {
        ...
        if (prop) {
          ...
          for (i = 0; i < dateInputs.length; i++)
            if (dateInputs[i].value && dateInputs[i].value.length) values.push(dateInputs[i].value);
        ...
        for (i = 0; i < inputText.length; i++) this.alterDateProperty(property.getSettings(), inputText[i]);
      ...
      if (disabled) {
        ...
        if (prop) {
          ...
          for (i = 0; i < fileLinks.length; i++) {
            ...
            if (html.length) values.push(html);
          }
        ...
        if (saved) propContainer = saved.querySelector("div.soa-property-container");

        if (propContainer) propsItemNode.appendChild(propContainer);
        else {
          propContainer = BX.create("DIV", { props: { className: "soa-property-container" } });
          property.appendTo(propContainer);
          propsItemNode.appendChild(propContainer);
    ...
    insertStringProperty: function (property, propsItemNode, disabled) {
      ...
      if (disabled) {
        ...
        if (property.getSettings().IS_PHONE === "Y") {
          this.bindValidationPhone(property.getId(), propContainer);
        } else {
          this.bindValidation(property.getId(), propContainer);
        }
      }
    ...
    insertEnumProperty: function (property, propsItemNode, disabled) {
      ...
      if (disabled) {
        ...
        if (prop) {
          ...
          if (inputs.length) {
            for (i = 0; i < inputs.length; i++) {
              if (inputs[i].checked) values.push(inputs[i].nextSibling.nodeValue);
            }
          }
          inputs = prop.querySelectorAll("option");
          if (inputs.length) {
            for (i = 0; i < inputs.length; i++) {
              if (inputs[i].selected) values.push(inputs[i].innerHTML);
            }
          }
    ...
    insertNumberProperty: function (property, propsItemNode, disabled) {
      ...
      if (disabled) {
        ...
        if (prop) {
          ...
          for (i = 0; i < inputs.length; i++) if (inputs[i].value.length) values.push(inputs[i].value);
        ...
        BX.bind(
          ...
          BX.delegate(function (e) {
            ...
            if (textInputs.length) {
              ...
              if (settings.TYPE == "DATE") this.alterDateProperty(settings, textInputs[textInputs.length - 1]);
    ...
    isValidForm: function () {
      ...
      if (regionErrors.length) {
        ...
        this.deliveryBlockNode.classList.remove("bx-step-completed");
        this.deliveryBlockNode.classList.remove("bx-selected");

        this.opened[this.deliveryBlockNode.id] = true;
      } else {
        const deliveryBlockAlertNode = this.deliveryBlockNode.querySelector(".alert.alert-danger");
        if (deliveryBlockAlertNode) {
          deliveryBlockAlertNode.style.display = "none";
          BX.cleanNode(deliveryBlockAlertNode);
        }
      ...
      return !(regionErrors.length + propsErrors.length + this.getCountValidationErrorInLicensesBlock());
    },
    ...
    isValidRegionBlock: function () {
      ...
      regionErrors = regionErrors.concat(this.isValidPropertiesBlock(false, this.deliveryBlockNode));
    ...
    getValidationDataPhone: function (arProperty, propContainer) {
      ...
      switch (arProperty.TYPE) {
        case "STRING":
          ...
          inputs = propContainer.querySelectorAll("input.phone");
          if ($(inputs).length) {
            data.inputs = inputs;
            break;
          }
    ...
    bindValidationPhone: function (id, propContainer) {
      ...
      if (data && data.inputs && data.action && "object" == typeof appAspro && appAspro && appAspro.phone) {
        for (let i = 0, c = data.inputs.length; i < c; ++i) {
    ...
    validatePhone: function (input, arProperty, fieldName) {
      ...
      return (
        ...
        "Y" == arProperty.IS_PHONE &&
          value.length > 0 &&
          (this.validateNormalPhone(input, errors, field), this.validateIntlPhone(input, errors, field)),
        errors.filter(Boolean)
      );
    },

    validateNormalPhone: function (input, errors, field) {
      arAsproOptions.THEME.PHONE_MASK &&
        arAsproOptions.THEME.VALIDATE_PHONE_MASK &&
        !appAspro.phone.checkIntlPhone &&
        (new RegExp(arAsproOptions.THEME.VALIDATE_PHONE_MASK).test($(input).val()) ||
          errors.push(field + " " + BX.message("JS_FORMAT_ORDER")));
    },

    validateIntlPhone: function (input, errors, field) {
      if (appAspro.phone.checkIntlPhone) {
        const telInput = $(input).data("iti");
        if (typeof telInput === "object" && telInput) {
    ...
    isValidPropertiesBlock: function (excludeLocation, node, notShowError) {
      if (!this.options.propertyValidation) return [];
      var blockNode = node || this.propsBlockNode;
      var props = blockNode.querySelectorAll(".bx-soa-customer-field[data-property-id-row]"),
        propsErrors = [],
        id,
        propContainer,
        arProperty,
      ...
      for (i = 0; i < props.length; i++) {
        ...
        if (propContainer) {
          ...
          propsErrors = propsErrors.concat(this.isValidProperty(data, true, notShowError));
        }
      }
    ...
    isValidProperty: function (data, fieldName, notShowError) {
      ...
      for (i = 0; i < data.inputs.length; i++) {
        ...
        if (inputErrors.length) propErrors[i] = inputErrors.join("<br>");
      }
      if (!notShowError) {
    ...
    getValidationData: function (arProperty, propContainer) {
      if (!arProperty || !propContainer) return;
      ...
      switch (arProperty.TYPE) {
        ...
        case "ENUM":
          ...
          if (!inputs.length) inputs = propContainer.querySelectorAll("input[type=checkbox]");
    ...
    getCountValidationErrorInLicensesBlock: function () {
      if (!this.licensesConditions.length) return 0;
    ...
    showErrorTooltip: function (tooltipId, targetNode, text) {
      if (!tooltipId || !targetNode || !text) return;
      ...
      if (tooltip) {
        ...
        if (quickLocation) targetNode = quickLocation;
    ...
    showValidationResult: function (inputs, errors) {
      ...
      if (label) tooltipId = label.getAttribute("for");

      for (i = 0; i < inputs.length; i++) {
        inputDiv = BX.findParent(inputs[i], { tagName: "DIV", className: "form-group" });
        if (errors[i] && errors[i].length) BX.addClass(inputDiv, "has-error");
        else BX.removeClass(inputDiv, "has-error");
      }

      if (errors.length) this.showErrorTooltip(tooltipId, label, errors.join("<br>"));
      else this.closeErrorTooltip(tooltipId);
    },
    ...
    validateString: function (input, arProperty, fieldName) {
      ...
      var value = input.value,
        ...
        // field = BX.message("SOA_FIELD"),
        re;

      if (arProperty.MULTIPLE === "Y") return errors;
      ...
      if (value.length) {
        ...
        if (value.length > 0 && arProperty.PATTERN && arProperty.PATTERN.length) {
          ...
          if (!re.test(value)) errors.push(field + " " + BX.message("SOA_INVALID_PATTERN"));
        }
      }
    ...
    validateLocation: function (input, arProperty, fieldName) {
      ...
      var parent = BX.findParent(input, { tagName: "DIV", className: "form-group" }),
        ...
        errors = [],
        field = !!fieldName
          ? BX.message("SOA_FIELD") + ' "' + BX.util.htmlspecialchars(arProperty.NAME) + '"'
          : BX.message("SOA_FIELD");

      if (arProperty.MULTIPLE == "Y" && arProperty.IS_LOCATION !== "Y") return errors;
    ...
    validateCheckbox: function (input, arProperty, fieldName) {
      ...
      if (arProperty.REQUIRED == "Y" && !input.checked) errors.push(field + " " + BX.message("SOA_REQUIRED"));
    ...
    validateNumber: function (input, arProperty, fieldName) {
      ...
      if (value.length) {
        if (!/[0-9]|\./.test(value)) errors.push(field + " " + BX.message("SOA_NOT_NUMERIC"));
        ...
        if (arProperty.STEP && parseFloat(arProperty.STEP) > 0) {
          num = Math.abs(
            parseFloat(value) - (arProperty.MIN && parseFloat(arProperty.MIN) > 0 ? parseFloat(arProperty.MIN) : 0)
          );
          del = (num / parseFloat(arProperty.STEP)).toPrecision(12);
          if (del != parseInt(del)) errors.push(field + " " + BX.message("SOA_NUM_STEP") + " " + arProperty.STEP);
        }
      }
    ...
    validateEnum: function (inputs, arProperty, fieldName) {
      ...
      for (i = 0; i < inputs.length; i++) if (inputs[i].checked || inputs[i].selected) values.push(i);

      if (arProperty.REQUIRED == "Y" && values.length == 0) errors.push(field + " " + BX.message("SOA_REQUIRED"));
    ...
    validateSelect: function (inputs, arProperty, fieldName) {
      ...
      for (i = 0; i < inputs.length; i++) if (inputs[i].selected) values.push(i);
    ...
    validateFile: function (inputs, arProperty, fieldName) {
      ...
      if (
        ...
        for (i = 0; i < files.length; i++) {
          ...
          if (
            arProperty.ACCEPT.length > 0 &&
            (fileExtension.length == 0 || arProperty.ACCEPT.indexOf(fileExtension) == "-1")
          )
            errors.push(
              BX.message("SOA_BAD_EXTENSION") +
                ' "' +
                fileName +
                '" (' +
                BX.util.htmlspecialchars(arProperty.ACCEPT) +
                ")"
            );

          if (file.size > parseInt(arProperty.MAXSIZE))
            errors.push(
              BX.message("SOA_MAX_SIZE") + ' "' + fileName + '" (' + this.getSizeString(arProperty.MAXSIZE, 1) + ")"
            );
        }
      }
    ...
    editTotalBlock: function () {
      ...
      if (this.options.showPayedFromInnerBudget) {
        this.totalInfoBlockNode.appendChild(
          this.createTotalUnit(BX.message("SOA_SUM_IT"), total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED, {
            total: true,
          })
        );
        this.totalInfoBlockNode.appendChild(
          this.createTotalUnit(
            BX.message("SOA_SUM_PAYED"),
            '<span class="payed">' + total.PAYED_FROM_ACCOUNT_FORMATED + "</span>"
          )
        );
        /* this.totalInfoBlockNode.appendChild(
          this.createTotalUnit(BX.message("SOA_SUM_LEFT_TO_PAY"), total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED, {
            total: true,
          })
        );*/
      } else {
        this.totalInfoBlockNode.appendChild(
          this.createTotalUnit(BX.message("SOA_SUM_IT"), total.ORDER_TOTAL_PRICE_FORMATED, { total: true })
        );
      }

      if (this.result.SERVICES_IN_ORDER) {
        this.totalInfoBlockNode.appendChild(
          this.createTotalUnit(
            BX.message("ORDER_ITEMS_TITLE") + ", " + total.ITEMS_COUNT + BX.message("ORDER_MEASURE_TITLE"),
            total.ITEMS_SUMM,
            params
          )
        );
        this.totalInfoBlockNode.appendChild(
          this.createTotalUnit(
            BX.message("ORDER_SERVICES_TITLE") +
              ", " +
              this.result.SERVICES_ITEMS.COUNT +
              BX.message("ORDER_MEASURE_TITLE"),
            this.result.SERVICES_ITEMS.SUMM_FORMATTED,
            params
          )
        );
      } else {
        totalName = BX.message("SOA_SUM_SUMMARY");
      ...
      if (deliveryError) {
        ...
        /*if (
          curDelivery &&
          typeof curDelivery.DELIVERY_DISCOUNT_PRICE !== "undefined" &&
      ...
      if (parseFloat(total.PAY_SYSTEM_PRICE) >= 0 && this.result.DELIVERY.length) {
        this.totalInfoBlockNode.appendChild(
          this.createTotalUnit(BX.message("SOA_PAYSYSTEM_PRICE"), "~" + total.PAY_SYSTEM_PRICE_FORMATTED)
        );
      }

      if (!this.result.SHOW_AUTH) {
        if (this.params.SHOW_COUPONS === "Y") {
          this.editCoupons(this.totalInfoBlockNode);
        }
        ...
        this.totalInfoBlockNode.appendChild(
          BX.create("DIV", {
            props: {
              className: "bx-soa-cart-total-button-container lic_condition" + (!showOrderButton ? " visible-xs" : ""),
            },
            children: [
              BX.create("A", {
                props: {
        ...
        const createCheckboxContainer = ({ type = "", name, error, text, checked = false }) => {
          return BX.create("div", {
            props: {
              className: `${type} form-checkbox mb mb--16 relative`,
            },
            children: [
              BX.create("label", {
                props: {
              ...
              BX.create("label", {
                ...
                children: [
                  BX.create("span", {
                    props: {
                      className: 'form-checbox__text',
                    },
                    html: text,
                  }),
                  BX.create("span", {
                    props: {
                      className: 'form-checkbox__box form-box',
                    },
                  }),
                ],
              }),
        ...
        const getCheckboxContainer = (htmlText = "") => {
        ...
        if (!this.licensesConditions.length) {
          if (arAsproOptions["THEME"]["SHOW_LICENCE"] == "Y" && appAspro.userConsent && appAspro.userConsent.licenceBlock) {
            this.licensesConditions.push(getCheckboxContainer(appAspro.userConsent.licenceBlock));
          }
          if (arAsproOptions["THEME"]["SHOW_OFFER"] == "Y" && appAspro.userConsent && appAspro.userConsent.offerBlock) {
            this.licensesConditions.push(getCheckboxContainer(appAspro.userConsent.offerBlock));
          }
        }

        if (this.licensesConditions.length) {
          if (!this.totalBlockNode.querySelector(".bx-soa-cart-conditions")) {
            this.totalBlockNode.querySelector(".bx-soa-total-wrapper").appendChild(
              BX.create("DIV", {
                props: {
                  className: "bx-soa-cart-conditions mt mt--24",
                },
                children: [
                  BX.create("div", {
                    props: {
                      className: "bx-soa-cart-conditions-text font_xs ",
                    },
                    children: this.licensesConditions,
                  }),
                ],
              })
            );
            ...
            checkboxes.forEach((checkbox) => {
              BX.bind(checkbox, 'change', function(e) {
                const $node = e.target;
                if ($node.checked) {
                  const oterWrap = $node.closest('.form-checkbox');
                  if (oterWrap) {
                    const errorLabel = oterWrap.querySelector("label.error");
                    if (errorLabel) {
    ...
    editMobileTotalBlock: function () {
      if (this.result.SHOW_AUTH) BX.removeClass(this.mobileTotalBlockNode, "visible-xs");
      else BX.addClass(this.mobileTotalBlockNode, "visible-xs");
      ...
      BX.bind(
        this.mobileTotalBlockNode.querySelector("a.btn-order-save"),
        "click",
        BX.proxy(this.clickOrderSaveAction, this)
      );
    },
    ...
    showTotalDeliveryInfo: function () {
      ...
      if (curDelivery) {
        this.totalInfoBlockNode.appendChild(
          this.createTotalScrollInfo(
            BX.message("ORDER_TOTAL_DELIVERY_TITLE"),
            curDelivery["NAME"],
            this.deliveryBlockNode
          )
        );

        if (curDelivery["PERIOD_TEXT"]) {
          ...
          if (removeChild) {
            for (let i = removeChild.length - 1; i > -1; i--) {
              BX.remove(removeChild[i]);
            }
          ...
          this.totalInfoBlockNode.appendChild(
            BX.create("div", {
              props: { className: "total-delivery-info font_13" },
              text: curDeliveryTotal, //curDelivery["PERIOD_TEXT"],
            })
          );
        }
      }
    ...
    showTotalPaymentInfo: function () {
      ...
      if (currentPaySystem) {
        this.totalInfoBlockNode.appendChild(
          this.createTotalScrollInfo(
            BX.message("ORDER_TOTAL_PAYMENT_TITLE"),
    ...
    createTotalScrollInfo: function (name, info, block) {
      return BX.create("div", {
        props: { className: "total-scroll-info font_13" },
        children: [
          BX.create("div", { props: { className: "total-scroll-info__title" }, text: name + ":" }),
          BX.create("div", {
            props: {
              className:
                "total-scroll-info__value" +
                (block.classList.contains("hidden") ? " " : " colored_theme_text_with_hover wborder"),
            },
            html: "<span>" + info + "</span>",
            events: {
              click: BX.delegate(function () {
                if (!block.classList.contains("hidden")) this.animateScrollTo(block);
              }, this),
            },
          }),
        ],
    ...
    createTotalUnit: function (name, value, params) {
      ...
      if (params.total) {
        className += " bx-soa-cart-total-line-totals";
      }
      ...
      return BX.create("DIV", {
        ...
        children: [
          ...
          BX.create("SPAN", {
            props: {
              className:
                "bx-soa-cart-d" + (!!params.total && this.options.totalPriceChanged ? " bx-soa-changeCostSign" : ""),
            },
            children: totalValue,
          }),
        ],
    ...
    basketBlockScrollCheckEvent: function (e) {
      ...
      if (scrollLeft == 0) BX.removeClass(parent, "bx-soa-table-fade-left");
      else BX.addClass(parent, "bx-soa-table-fade-left");

      if (scrollRight == 0) BX.removeClass(parent, "bx-soa-table-fade-right");
      else BX.addClass(parent, "bx-soa-table-fade-right");
    },
    ...
    basketBlockScrollCheck: function () {
      ...
      for (i = 0; i < scrollableNodes.length; i++) {
        ...
        tableNode = parentNode.querySelector("table.bx-soa-item-table");
        ...
        if (scrollable) {
          ...
          if (scrollLeft == 0) BX.removeClass(parentNode, "bx-soa-table-fade-left");
          else BX.addClass(parentNode, "bx-soa-table-fade-left");

          if (scrollRight == 0) BX.removeClass(parentNode, "bx-soa-table-fade-right");
          else BX.addClass(parentNode, "bx-soa-table-fade-right");

          if (scrollLeft == 0 && scrollRight == 0) BX.addClass(parentNode, "bx-soa-table-fade-right");
        } else BX.removeClass(parentNode, "bx-soa-table-fade-left bx-soa-table-fade-right");
      }
    },
    ...
    totalBlockScrollCheck: function () {
      ...
      var offset = 0;
      var headerFixed = BX("headerfixed");
      if (headerFixed) {
        if (BX.hasClass(headerFixed, "fixed")) {
          offset = headerFixed.offsetHeight + 20;
        }
      }
      ...
      if (scrollTop + offset > ghostTop && !BX.hasClass(this.totalInfoBlockNode, "bx-soa-cart-total-fixed")) {
      ...
      } else if (scrollTop + offset < ghostTop && BX.hasClass(this.totalInfoBlockNode, "bx-soa-cart-total-fixed")) {
    ...
    totalBlockFixFont: function () {
      ...
      if (objList.length) BX.FixFontSize.init({ objList: objList, onAdaptiveResize: true });
    },

    setAnalyticsDataLayer: function (action, id) {
      if (!this.params.DATA_LAYER_NAME) return;
    ...
    hasUserProfiles: function () {
      return !!(this.result.USER_PROFILES && BX.util.object_keys(this.result.USER_PROFILES).length);
    },
  };
})();
